...
- **Tool storage primitive** (Plan 1) handles the watering can lifecycle: `ToolStorages` list on `CommercialBuilding` (role-tagged storages; falls back to first-crate convention if none tagged), `OwnerBuildingId` stamp on fetch, AddItem origin-clear hook on return, `CharacterJob.CanPunchOut` gate at shift end. Worker-side resolution: `JobFarmer.ProvideWorldState` uses `farm.HasToolInAnyToolStorage(WateringCanItem)` so multiple tool storages all count toward the precondition; `GoapAction_FetchToolFromStorage` / `ReturnToolToStorage` re-resolve per call via `FindToolStorageContaining` / `FindToolStorageWithFreeSpace` (no cached InteractableObject — supports runtime role flips).
