...
- **Tool storage primitive** (Plan 1) handles the watering can lifecycle: `ToolStorages` list on `CommercialBuilding` (role-tagged storages; falls back to first-crate convention **only when no chest carries an explicit non-`None` role** — a chest the owner has tagged `InventoryStorage`/`SellShelf` is no longer eligible for the convention fallback, fixed 2026-05-14, see [[commercial-storage-roles]] change log), `OwnerBuildingId` stamp on fetch, AddItem origin-clear hook on return, `CharacterJob.CanPunchOut` gate at shift end. Worker-side resolution: `JobFarmer.ProvideWorldState` uses `farm.HasToolInAnyToolStorage(WateringCanItem)` so multiple tool storages all count toward the precondition; `GoapAction_FetchToolFromStorage` / `ReturnToolToStorage` re-resolve per call via `FindToolStorageContaining` / `FindToolStorageWithFreeSpace` (no cached InteractableObject — supports runtime role flips).
- **Seed discovery** (refactored 2026-05-14) — `SeedRegistry` (mirror of `CropRegistry`) walks `Resources/Data/Item/Seed/` at boot and indexes SeedSOs by their `_cropToPlant` back-link. `FarmingBuilding.HasSeedInInventory` / `SeedForCropExists` / `GetStockTargets` and `GoapAction_FetchSeed.ResolveSeedFor` all call `SeedRegistry.GetSeedFor(crop)` instead of walking `crop._harvestOutputs`. This decouples planting discovery from harvest-yield semantics: a tree crop can list `[Apple]` only in `_harvestOutputs` and `[Wood, Sapling]` in `_destructionOutputs` without the system silently failing to find its seed.
...
- **Seed discovery rides `SeedSO._cropToPlant`, not `CropSO._harvestOutputs`** (decoupled 2026-05-14). `FarmingBuilding.HasSeedInInventory` / `SeedForCropExists` / `GetStockTargets` and `GoapAction_FetchSeed.ResolveSeedFor` all route through `SeedRegistry.GetSeedFor(crop)` — a boot-time `Resources.LoadAll<SeedSO>("Data/Item/Seed")` walk indexed by `_cropToPlant`. Designer fix when a farmer can't plant: open the SeedSO asset and set `_cropToPlant` to the matching CropSO. The CropSO's `_harvestOutputs` / `_destructionOutputs` are **pure yield** — designers configure them for gameplay meaning (apples on harvest, saplings on destruction) without worrying about feeding the planting pipeline. As of 2026-05-14 a throttled `LogWarning` (5 s per building) fires when `SeedRegistry.GetSeedFor` returns null, naming the misauthored CropSO. See [farming SKILL](../farming/SKILL.md#debug-a-farmer-that-wont-plant-despite-seeds-visibly-in-storage) for the fix recipe. Real example: `AppleTreeSO` shipped with `[Item_Apple]` only (no seed entry) and farmers were stuck Idle until both the SeedRegistry refactor + the SeedSO back-link were in place.
