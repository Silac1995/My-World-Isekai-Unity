...
- Once `t01 >= 1f` (the hold threshold), the controller **flips `_eMenuOpened = true` first** so the subsequent KeyUp early-exits without firing the tap path, **then** dispatches the unified hold-menu via `_detector.TriggerHoldMenu(_detector.CurrentTarget)` — which pulls `GetHoldInteractionOptions(Character)` from the target and routes through `PlayerUI.OpenInteractionMenu`.
- One menu, one renderer, one input path for every interactable type: doors, NPCs, harvestables, furniture. The options (e.g., *Pick Apple*, *Destroy*, *Follow Me*, *Greet*, *Unlock*) are displayed in the shared interaction menu via `PlayerUI`.

**Harvestables in the unified flow:** `Harvestable.GetHoldInteractionOptions` is an adapter that calls the richer `Harvestable.GetInteractionOptions(Character)` (which still returns `IList<HarvestInteractionOption>` with Label / Icon / OutputPreview / IsAvailable / UnavailableReason / ActionFactory — the internal source of truth) and flattens each row into the global `InteractionOption` shape (Name + IsDisabled + Action). UnavailableReason is appended to the label in parentheses so the player still sees the missing-tool hint (`"Pick Apple (Requires Axe)"`) without a dedicated rich-row UI. The previous bespoke `UI_HarvestInteractionMenu` / `UI_HarvestInteractionOptionRow` / their prefabs are deleted.

**"Hold doesn't tap" guarantee:** flipping `_eMenuOpened` at threshold-cross is unconditional — it does NOT depend on a menu actually opening. So holding E past 0.4s on a target with no `GetHoldInteractionOptions` (e.g. a generic door) consumes the press cleanly: no menu, no tap on release. Without this, a generic door + 0.5s hold would still fire `target.Interact(Character)` on release because `_eMenuOpened` would have stayed `false`. The flag is reset on every next `HandleEKeyDown` (line: `_eMenuOpened = false;`).
