...
- `UI_RemoteActionIndicator` + `RemoteActionIndicatorLayer` (2026-05-19) — per-non-local-character variant. The singleton manager spawns one indicator per remote `Character` via `Character.OnCharacterSpawned/Despawned` static events; each indicator follows its target's head, fades with distance to the local player, and shows a Radial360 progress arc instead of a linear bar so it doesn't compete with the player's own bar. PlayerPrefs-backed toggle `MWI.HUD.ShowRemoteActionBars` (default off), flipped via `/togglebars` chat command.
