...
**Idempotency:** per-storage write routes through `CommercialBuilding.TrySetStorageRoleServer(storage, desired)` which early-outs on `storage.Role == desired`. Replays cost zero replication traffic. **Server-only:** the helper calls into `DoSetStorageRole` → `StorageFurnitureNetworkSync.SetRoleServer` — same path the player RPC uses (2026-05-14: both paths converged through `DoSetStorageRole`); the NetVar fans out via `OnValueChanged` → `OnRoleChanged` → `CommercialBuilding.OnStorageRolesChanged`. **Subtype-safe:** the `TrySetStorageRoleServer` filter rejects roles outside `SupportedStorageRoles` (same rule the player RPC enforces), returns `false`, and the NPC pass skips logging — the helper logs its own warning. **Caller gate (2026-05-17):** the auto-assign pass is gated INSIDE `CommercialBuilding.WorkerStartingShift` to ONLY fire when the punching-in worker is the building's `JobLogisticsManager` NPC (matched via `worker.CharacterJob.ActiveJobs.OfType<JobLogisticsManager>().FirstOrDefault(j => j.Workplace == this) != null`). Vendors, farmers, and other non-LogisticsManager workers punching in run NEITHER `AssignStorageRolesForShift` nor `AssignSafeRolesForShift`.

**Owner ↔ LogisticsManager role-mgmt contract (by design):** two parties touch storage / safe roles — the owner (via the management console's `TrySetStorageRoleServerRpc` / `TrySetSafeRoleServerRpc` dropdowns) and the building's LogisticsManager NPC (via the gated `AssignStorageRolesForShift` / `AssignSafeRolesForShift` passes on shift-punch). Both write through the same canonical `DoSetStorageRole` / `SafeFurnitureNetworkSync.SetRoleServer` mutators. The LogisticsManager's policy re-evaluation on shift-punch CAN overwrite owner choices that disagree with the policy — this is intended ("no win forever"). If the owner wants a role to stick, they must keep it consistent with the policy, fire the LogisticsManager, or pick an off-policy role and re-set it after each LogisticsManager shift. No `_roleLocked` flag.
...
- **Auto-assign** — `BuildingLogisticsManager.AssignSafeRolesForShift()` (sibling of `AssignStorageRolesForShift`, called from `WorkerStartingShift`) flips every `Role == None` safe to `Treasury` on every NPC LogisticsManager shift-punch. Idempotent. **Routes through `building.TrySetSafeRoleServer` → `DoSetSafeRole`** (2026-05-17b convergence) so the NPC path and the player UI path share identical side-effects.
...
- **Reputation visible surfaces (2026-05-17f)** — three places now show the building's current reputation: (a) DevMode Inspect → `BuildingOverviewSubTab` colour-coded read-out in the Commercial section; (b) DevMode [DEV] Console Management → `BuildingConsoleManagementSubTab` "Reputation" section with −10 / −1 / +1 / +10 nudge buttons wired to the new `CommercialBuilding.DevForceChangeReputation(int delta)` mutator (host+dev-mode-gated, routes through `TryChangeReputation` for clamp + event + NetVar + audit log); (c) in-game Management Panel → `HiringTabView` optional `_reputationLabel` SerializeField with live `OnReputationChanged` subscription (null-tolerant — older HiringTab prefab variants still work without re-authoring). Colour-coding convention reused across all three: green ≥ `ReputationB2BMinimum` (20), amber 1–19, red 0.
- **NPC-buys-from-building convention (2026-05-17e, picker formula rev 2026-05-17g)** — every NPC decision to buy goods or use a service from a `CommercialBuilding` MUST route the final pick through `ReputationWeightedPicker.Pick<T>(IList<T>) where T : CommercialBuilding`. **Pool-relative floor formula:** `floor = max(AbsoluteFloor=10, maxRepInPool / RelativeFloorDivisor=10); weight = max(floor, building.Reputation)`. Preserves the "lowest candidate keeps ≥10% relative weight vs the leader" invariant at any scale, including the unbounded high end (rep has no upper cap since 2026-05-17g). **Two-pass shape** for new consumers — Pass 1 collects qualifiers (catalog match / stock / gates / subtype-specific hard floors like `ReputationB2BMinimum=20` for procurement), Pass 2 picks weighted. Procurement-side callers apply the hard floor; customer-side callers don't (every qualifying shop competes by weight only). Canonical sites: `GoapAction_GoShopping.IsValid` (customer-NPC visits) + `LogisticsStockEvaluator.TryB2BPurchaseFromShop` (B2B procurement). Server-only — `UnityEngine.Random.Range` is per-peer; all NPC decision logic runs server-authoritatively. Full architectural treatment in [[commercial-treasury]] §Reputation.
- **Owner UI (Phase 1.7 — 2026-05-17b)** — `StorageRolesTab` (same tab the owner already uses for storages) now hosts a parallel **Safes** section: one `StorageRolesTabSafeRow` per `SafeFurniture` child with a TMP_Dropdown bound to `CommercialBuilding.SupportedSafeRoles` (new virtual returning `SafeRoleCatalog.Generic`; future Bank / Vault subtypes can widen). Dropdown change fires `CommercialBuilding.TrySetSafeRoleServerRpc(furnitureRef, newRole)` — owner-gated server-side (mirror of `TrySetStorageRoleServerRpc` exactly: `caller != Owner` → reject + warn; `newRole ∉ SupportedSafeRoles` → reject + warn). Tab tree extended to `StoragesHeader / RowsParent / EmptyStateLabel / SafesHeader / SafesRowsParent / SafesEmptyStateLabel`. The view subscribes to `building.OnTreasuryChanged` for full-section repaint; each row also subscribes to per-safe `OnRoleChanged` + `OnBalanceChanged` for live label refresh on deposits / withdraws / B2B / refunds without rebuilding the section. Empty-state copy: "Place a Safe Base furniture inside to track treasury." Late-joiner safe via existing per-safe `NetworkVariable` + `NetworkList` (no new replication channel introduced).
