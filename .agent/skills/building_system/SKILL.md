...
- **Lock-state replication (shipped 2026-05-09):** `StorageFurnitureNetworkSync` also carries `NetworkVariable<bool> _isLockedSync` (server-write, everyone-read, default `false`). `HandleServerInventoryChanged` pushes `_storage.IsLocked` into the variable whenever it differs — since `Lock()` / `Unlock()` both fire `OnInventoryChanged`, this handler is the single propagation chokepoint (no separate lock event needed). `HandleLockChanged(bool prev, bool next)` is the shared `OnValueChanged` callback for both server and client; it calls `_storage.ApplyLockStateFromNetwork(next)`. That method sets `_isLocked` directly (bypassing `Lock()` / `Unlock()`) and fires `OnInventoryChanged` once. The early-return when the value matches prevents feedback loops. Server seeds `_isLockedSync.Value = _storage.IsLocked` in `OnNetworkSpawn`; clients apply the current value in `OnNetworkSpawn` for late-join parity. Unsubscribed in `OnNetworkDespawn`.
