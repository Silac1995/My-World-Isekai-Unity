...
- **Do NOT write owner-gates as `building.Owner == character` / `building.Owner != character`.** The singular `Owner` getter returns only `_ownerIds[0]` and silently rejects every secondary owner (added via `CommercialBuilding.AddOwner` — the dev console's `[DEV] Add Owner` button or any future co-ownership flow). Always route through `building.IsOwner(character)` (the `Room`-level predicate that compares against the full replicated `_ownerIds` list). See [[wiki/gotchas/singular-owner-vs-multi-owner-isowner.md]] for the canonical fix recipe + audit list.
