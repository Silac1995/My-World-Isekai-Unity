...
7. **Ambition** (`BTAction_PursueAmbition` — added 2026-05-18): Drives the active `AmbitionQuest` step. Reads `CharacterAmbition.Current.CurrentStepQuest`, dispatches to per-task drivers — `Task_CreateCommunity`, `Task_PlaceBuilding`, `Task_PromoteCommunity` run inline (short atomic operations); **`Task_FinishConstruction` returns `BTNodeStatus.Failure` and lets the BT Selector fall through to GOAP** (the next priority), where `NeedAmbitionFinishConstruction` supplies a `{ambitionBuildingFinalized=true}` goal that `GoapAction_FulfillAmbitionConstruction` plans against — see [[ai-goap]] for the composite gather/carry/drop state machine. This means the inline-driven steps still preempt GOAP, but the long-running material-gather step shows in `CharacterGoapController` inspectors as a proper goal + action. Persistent state across ticks: `_targetHarvestable`, `_wanderDestination`, `_wanderUntilTime` (cleared on `OnEnter`/`OnExit` so a BT branch switch restarts cleanly). No new replication channel. See [[wiki/systems/city-management]] for the end-to-end ambition loop.
