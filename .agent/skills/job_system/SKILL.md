...
**CityHarvester runtime branch (STUBBED)**: `JobHarvester.Execute` now early-detects `_workplace is AdministrativeBuilding` and routes to `ExecuteCityHarvesterStub(ab)`. The stub clears any inherited harvest action, logs the unfulfillable queue contents (5 s throttle, gated behind `NPCDebug.VerboseJobs`), and idles. The full Physics.OverlapSphere scan + `CharacterHarvestAction` queue + walk-back-to-AB-storage cycle is a follow-up. Until then the city construction pipeline ships fully functional for the B2B + producer + virtual supplier paths; the harvest fallback queues but does not drain.
