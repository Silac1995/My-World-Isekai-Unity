...
- **Punching In/Out**: Handled by strict `CharacterAction`s (`Action_PunchIn` / `Action_PunchOut`) triggered from a physical `TimeClockFurniture` authored inside the building. Players press E on the clock → `TimeClockFurnitureInteractable.Interact` → (client) `CommercialBuilding.RequestPunchAtTimeClockServerRpc` → server runs the action. NPCs walk to `workplace.TimeClock.GetInteractionPosition()` in `BTAction_Work` / `BTAction_PunchOut` and call `Interact` directly (already server-side), sharing the player code path (Rule #22). `WorkerStartingShift` / `WorkerEndingShift` on `CommercialBuilding` now carry `!IsServer` guards (defence-in-depth); buildings with no clock authored yet fall back to the legacy "punch anywhere in `BuildingZone`" behaviour with a one-shot warning log.
