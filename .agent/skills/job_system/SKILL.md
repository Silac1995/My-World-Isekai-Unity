...
- **Auto-claim is player-only (2026-05-02)**: `CommercialBuilding.WorkerStartingShift` calls `TryAutoClaimExistingQuests` and `SubscribeWorkerQuestAutoClaim` only when `worker.IsPlayer()`. NPCs use GOAP's `ClaimBestTask` on demand instead. Without this gate, the first NPC to subscribe hoarded every newly-published `BuildingTask` via the multicast `OnQuestPublished` event order, leaving subsequent NPCs idle. Symptom seen pre-fix: a single farmer scooped every `PlantCropTask` registered by the day's `PlantScan` and the second farmer's `worldState` reported zero work. **NPCs should not register for player-flavoured auto-claim.**

- **worldState predicate ↔ action `IsValid` symmetry rule**: when a `Job` builds a `_scratchWorldState[key]` that the cascade reads to pick a goal, the predicate MUST mirror the consuming GOAP action's `IsValid` filter exactly — including blacklist, yield-match, and any other gate the action applies. Otherwise the cascade picks a goal whose plan can't form (the action is filtered out of `_scratchValidActions` by its own `IsValid`), and the worker freezes on that goal forever. Symptom: `worldState` insists `hasUnfilledHarvestTask=true` while `HarvestResources.IsValid` REJECTED dump shows `blacklisted=N`. See `wiki/gotchas/worldstate-predicate-action-isvalid-divergence.md` and `JobFarmer.PlanNextActions` for the canonical mirror.

- **"Lie to the planner" patterns MUST gate on actionable work** (2026-05-19): some `Job.PlanNextActions` deliberately set a worldState flag opposite to the worker's real state to force a longer plan chain (e.g. `JobHarvester` sets `hasResources=false` while the worker holds an item, so the planner picks `Harvest → Pickup` again instead of detouring to `Deposit` after every single unit). The lie is only safe **while the chain it forces can actually extend**. If the gather chain dries up (last destroy task completed, last loose item picked up, all remaining trees claimed by other workers, etc.), the lie blocks the planner from forming any chain to the deposit goal — `Deposit` needs `hasResources=true`, every other action needs `hasResources=false` AND `hasHarvestZone=true` AND/OR `looseItemExists=true`. Returned plan is null, worker freezes mid-zone holding the item. **Rule**: any "lie to the planner" branch must AND-gate on the task-availability flags it depends on (`canHarvest || looseItemExists` in `JobHarvester`'s case). When the lie's premise dries up, flip the lie off and let the planner pivot to deposit. Mirror `JobHarvester.PlanNextActions`'s 2026-05-19 fix shape — compute the task-availability scan above the lie, gate the lie on it, fall through to the honest branch (`hasResourcesForGoap = true`) when no work remains. See `wiki/gotchas/harvester-deposit-freeze.md`.

- **`PickupLooseItemTask` registration MUST filter by workplace accepted items** (2026-05-20): `CharacterActions.ApplyHarvestOnServer` and `ApplyDestroyOnServer` register a `PickupLooseItemTask` on the worker's workplace for every spawned `WorldItem`. Without a filter on `HarvestingBuilding.GetAcceptedItems()`, a multi-output harvestable (e.g. apple tree drops wood + apple sapling on destruction) registers tasks for ALL drops — including byproducts the workplace does NOT accept. `BuildingTaskManager.ClaimBestTask` may then return the byproduct task first; worker picks it up; `hasAtLeastOneResource` stays false on the next plan tick (byproduct not in `acceptedItems`); no deposit chain forms; worker freezes mid-zone holding an item the building won't accept. **Three-layer fix shape**: (1) `CharacterActions` caches `workplace.GetAcceptedItems()` once per harvest/destroy call and gates `RegisterTask` on `accepted.Contains(spawned.ItemSO)`; (2) `GoapAction_PickupLooseItem.Execute.ClaimBestTask` predicate filters by accepted items at Execute time (defense in depth — Layer 2 lives ONLY in Execute, NOT in IsValid, because Pickup is a CHAIN CONSUMER and pre-filtering its IsValid on "does a task exist now?" breaks the Harvest→Pickup→Deposit chain at punch-in when no item is dropped yet — see [[chain-action-isvalid-pre-filter]]); (3) the consuming `Job.PlanNextActions` `looseItemExists` worldState predicate filters by accepted items, mirroring Layer 1's registration filter (worldState is safe to filter — it reads CURRENT state, unlike IsValid which gates SIMULATED chain extensions). Non-wanted byproducts stay on the ground for players to pick up. See `wiki/gotchas/harvester-picks-wrong-loose-item.md`.

- **Chain-action `IsValid` must NOT pre-filter by carry state**: GOAP actions that consume hand state (PlantCrop, WaterCrop, ReturnToolToStorage, etc.) are *chain consumers* — the planner is supposed to add the matching Fetch action by walking preconditions. Pre-filter via `IsValid` runs BEFORE the planner sees them, so a `IsValid` that requires "seed in hand" gets the action filtered out before any plan can form. **Rule**: chain-action `IsValid` only checks invariants the planner cannot deduce from preconditions (workplace exists, tasks exist, hands controller exists, etc.). The "seed in hand" / "can in hand" check belongs to the precondition dict, NOT to `IsValid`. See `wiki/gotchas/chain-action-isvalid-pre-filter.md`.

See `.agent/skills/quest-system/SKILL.md` and `wiki/systems/quest-system.md` for the full architecture.

## JobBuilder + CityHarvester (Plan 4b, 2026-05-18)

**[JobBuilder](../../Assets/Scripts/World/Jobs/BuilderJobs/JobBuilder.cs)** is the city-construction NPC. Employed exclusively at [AdministrativeBuilding](../../Assets/Scripts/World/Buildings/CommercialBuildings/AdministrativeBuilding.cs) (`CanExecute` gates on `_workplace is AdministrativeBuilding`). Mirrors JobFarmer's shape verbatim — `ExecuteIntervalSeconds = 0.3f`, scratch `worldState` dict, `_scratchValidActions` IsValid pre-filter, cached goals, force-replan on action completion, 1 Hz no-plan diagnostic dump. Goal cascade (high → low):
1. **DeliverAndConstruct** (priority 5) — carrying a needed material AND active `BuildOrder` → plan picks `GoapAction_GoToConstructionSite` → `GoapAction_DropMaterialAtZone` → `GoapAction_FinishBuildingConstruction`. Multi-trip handled by force-replan on `IsComplete`.
2. **FetchFromABStorage** (priority 4) — active `BuildOrder` AND any missing material is in the AB's `StorageFurniture` chain → plan picks `GoapAction_TakeMaterialFromABStorage`.
3. **Idle** (implicit) — no achievable goal → worker stands at workplace. JobFarmer's `GoapAction_IdleInBuilding` takes `HarvestingBuilding` so it cannot reuse here; the implicit no-plan path is fine for v1.

**The four NEW GoapActions** all live in `Assets/Scripts/AI/GOAP/Actions/` and take `AdministrativeBuilding` in their constructor; each reads `_ab.LogisticsManager.GetFirstActiveBuildOrder()` to anchor its target. Movement gates follow rule #36 (`IsCharacterInInteractionZone` for storage, 2D X-Z bounds for `BuildingZone`, softlock guard + path-loss re-fire across the board). Drop + FinishConstruction wrap their `CharacterAction` via `OnActionFinished` subscription so the GOAP planner waits for the underlying animation.

**[BuildOrder : IQuest](../../Assets/Scripts/World/Jobs/BuildOrder.cs)** is the order type JobBuilder consumes. Lives on `BuildingLogisticsManager._activeBuildOrders` (server-only `List`). Non-expiring (`IsExpired = false`, `RemainingDays = -1`). `IsCompleted` reads off `TargetBuilding.IsUnderConstruction`. Implements `IQuest` so it appears in the standard quest pipeline (Issuer = `ClientBoss`, Target = `BuildingTarget(TargetBuilding)`).

**Supply chain**: `JobLogisticsManager.Execute` calls `_workplace.LogisticsManager.ProcessActiveBuildOrders()` each tick (alongside `ProcessActiveBuyOrders`). The new pass iterates active BuildOrders, computes per-material shortfall (`missing - inStorage - inFlight`), and routes the gap through `Evaluator.RequestStock(item, qty)` — which cascades B2B → producer chain → virtual supplier internally. `RequestStock` now returns `bool` (was `void`); on `false` (no supplier of any tier exists) AND building is an `AdministrativeBuilding`, `ab.EnqueueUnfulfillableMaterial(item, qty)` lands the request on the AB's `_unfulfillableMaterialHarvestQueue`. JobHarvester's CityHarvester branch is supposed to drain that queue via physical harvest.

**CityHarvester runtime branch (FULL)**: `JobHarvester.Execute` early-detects `_workplace is AdministrativeBuilding` and routes to `ExecuteCityHarvesterTick(ab)`. The state machine has seven states — `Idle → FindTarget → MoveToTarget → Harvesting → PickupDroppedItem → MoveToABStorage → DepositItem → Idle` — wrapping the standard `CharacterHarvestAction` + `CharacterPickUpItem` + `CharacterStoreInFurnitureAction` primitives directly (the existing GOAP actions `GoapAction_HarvestResources` / `PickupLooseItem` / `DepositResources` take `HarvestingBuilding` in their constructors and can't be reused for AB). FindTarget runs a `Physics.OverlapSphereNonAlloc` 30 u scan filtered by `Harvestable.HasYieldOutput` + blacklist + `CanHarvest`; PickupDroppedItem scans a 2 m radius around the worker (matches the spawn anchor in `CharacterActions.ApplyHarvestOnServer`). On successful deposit, calls `ab.AddToInventory` + `ab.DecrementUnfulfillableMaterial(wanted, 1)`. Failure modes: no Harvestable in range → 2 s cooldown + throttled diagnostic; storage full → drops logically via AddToInventory + worker-side remove; action rejected → ClearCurrentAction + reset. Pipeline never softlocks; end-to-end harvest fallback now ships.

**AdministrativeBuilding.InitializeJobs** populates `_jobs` with `JobBuilder × 2 + JobHarvester + JobLogisticsManager`. The harvester's CityHarvester variant is auto-detected by the workplace cast at runtime — no JobType / JobCategory split required.

**Sources**: see `wiki/systems/administrative-building.md` Change log 2026-05-18 entry + `wiki/systems/construction.md` Change log 2026-05-18 entry. Implementation plan: `docs/superpowers/plans/2026-05-18-job-builder-and-goap-actions.md`. Spec: `docs/superpowers/specs/2026-05-18-city-founding-and-administrative-building-design.md`.
