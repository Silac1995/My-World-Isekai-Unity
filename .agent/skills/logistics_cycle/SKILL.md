...
### 3c-bis. Sell-shelf restock loop (shop-only intra-building rebalance)

`ShopBuilding` declares a sale `Catalog` (`IReadOnlyList<ShopItemEntry>`) authored on the prefab. Items in the catalog are the ones for sale; everything else in storage is just inventory the shop happens to own. Two related code paths now keep the catalog items routed to the right shelf:

1. **Routing-side preference (new deliveries)** — `CommercialBuilding.FindStorageFurnitureForItem` adds a `SellShelf` pre-pass between the tool pre-pass and the generic first-fit walk. The branch fires when the building is a `ShopBuilding` AND the item is in `shop.GetCatalogEntry(item.ItemSO) != null`. Iterates `GetStoragesWithRole(StorageRoleType.SellShelf)` and returns the first unlocked shelf with free space. Falls through to the generic walk when shelves are all full / locked / absent. Tool items are still routed by the earlier tool pre-pass (tool priority wins). Mirrored inside `GoapAction_GatherStorageItems.DetermineStoragePosition` since that action does its own walk (it needs `_excludedFurniture` filtering and can't call `FindStorageFurnitureForItem` directly).
   - **Invariant (2026-05-15):** the generic first-fit walk in BOTH `FindStorageFurnitureForItem` AND `GatherStorageItems.DetermineStoragePosition` now skips `Role == SellShelf` explicitly — symmetric to the existing tool-storage exclusion for non-tool items. Without this, a non-catalog item (e.g. a weapon dropped inside a bread shop) fell through the catalog-gated pre-pass and first-fit-landed on a SellShelf with any matching free slot, polluting the sale display with items the shop doesn't sell. SellShelves are now reserved exclusively for the catalog pre-pass; non-catalog items can only reach an InventoryStorage chest (or fall back to a loose `StorageZone` drop when no inventory chest has room).

2. **Heal-existing-state action (existing misplaced stock)** — new `GoapAction_RestockSellShelves` (`Cost = 0.3f`) in the `JobLogisticsManager` plan. State machine: `Finding → MovingToSource → TakingFromSource → MovingToShelf → StoringOnShelf`. Walks `_shop.GetItemsInStorageFurniture()` looking for a triple `(sourceFurniture, slotItem, targetShelf)` where source's role is NOT SellShelf, slot item is in `shop.Catalog`, AND at least one SellShelf has free space. Takes via `CharacterTakeFromFurnitureAction`, then stores via `CharacterStoreInFurnitureAction` — same actions the player UI uses. Re-validates slot contents at `TakingFromSource` (another worker may have grabbed the item) and shelf capacity at `StoringOnShelf` (shelf may have filled). 5-second move timeout with per-instance exclusion sets for both source and shelf. **Reservation-aware** — skips slot items currently reserved by an outbound `TransportOrder` (`Source == _shop && !IsCompleted && t.ReservedItems.Contains(instance)`); those belong to `GoapAction_StageItemForPickup`'s domain.

Planner cost ordering: `PlaceOrder 0.1 < StageForPickup 0.2 < RestockSellShelves 0.3 < GatherStorageItems 0.5 < Idle`. The planner picks restock before generic gather but after outbound staging, which means: an outbound transport with a reserved-but-misplaced item gets staged out before restock would try to move it; new supplier deliveries stop on the floor (BuildingZone) get gathered into the right shelf directly via path (1) without restock ever firing; mid-shift, items inadvertently left in InventoryStorage (or items that started there from earlier shifts before SellShelf existed) get migrated by restock until shelves saturate.

**Idempotency** — the planner re-fires `IsValid` between actions; restock completes one slot transfer per planning cycle and yields back to the goal. The next plan picks restock again if more slots are still misplaced, or falls through to `GatherStorageItems` / `Idle` once shelves are full or sources empty. Multiple workers on the same shift naturally pick distinct items (the second worker's `IsValid` scan sees the slot already emptied by the first).

**No `TransportOrder` schema change** — same-building intra-storage moves don't model as transport. The action drives the worker via raw `CharacterAction`s; the source slot is drained on `TakingFromSource` and the target slot is filled on `StoringOnShelf`. Logical inventory (`_inventory`) is not touched because the slot mutations leave the building's logical state intact — the item never leaves the building.
...
2026-05-14 — Dirty-flag gating for `GoapAction_RestockSellShelves` (rule #34). New server-only `_restockDirty` field on `BuildingLogisticsManager` + `IsRestockDirty` / `MarkRestockDirty` / `ClearRestockDirty` API mirroring `LogisticsOrderBook._dispatchDirty`. `IsValid` early-exits with a single field read on stable state; the expensive `GetItemsInStorageFurniture × FindShelfWithSpace` walk only runs when one of: (a) `CommercialBuilding.AddToInventory` / `TakeFromInventory` / `RemoveExactItemFromInventory` fired (sibling to existing `MarkDispatchDirty` calls), (b) any building storage's `OnInventoryChanged` fired (per-storage subscriptions managed through the existing `GetStorageFurnitureCached` chokepoint via new `BuildingLogisticsManager.RefreshStorageContentSubscriptions`), (c) `OnStorageRolesChanged` or (shop) `OnCatalogChanged` fired (subscribed once in `BuildingLogisticsManager.Awake`), or (d) the action itself completed a slot transfer (sets dirty so more candidates are re-scanned). Carried-item early-return runs before the gate (must complete in-flight transfers regardless of cleanliness). Initial value = dirty (warm-start / load-from-save). Server-only state, no replication.

2026-05-14 — Sell-shelf restock loop. (1) Routing — `CommercialBuilding.FindStorageFurnitureForItem` and `GoapAction_GatherStorageItems.DetermineStoragePosition` add a SellShelf pre-pass when the building is a `ShopBuilding` AND the item is in `shop.GetCatalogEntry`. Catalog items now route onto a SellShelf on first deposit. (2) Heal — new `GoapAction_RestockSellShelves` (cost 0.3f) wired into `JobLogisticsManager._availableActions`. Sweeps `GetItemsInStorageFurniture()` for catalog items sitting in non-SellShelf storage, takes via `CharacterTakeFromFurnitureAction`, stores via `CharacterStoreInFurnitureAction` on a SellShelf with free space. Reservation-aware (skips items reserved by outbound `TransportOrder` — defers to `GoapAction_StageItemForPickup`). Server-authoritative through the existing actions, idempotent (re-fires until shelves are full or sources empty), no `TransportOrder` schema change. See section 3c-bis for the full state machine and cost ordering.

2026-05-09 — Multi-tool-storage refactor: `CommercialBuilding.ToolStorages` (list) is now the canonical accessor; `FindStorageFurnitureForItem` iterates every role-tagged tool storage for tools, skips every tool storage (via `IsToolStorage`) for non-tools. `GoapAction_GatherStorageItems.DetermineStoragePosition` uses the same iteration. The singleton `ToolStorage` accessor is preserved with a two-tier resolver (role-tagged → first-crate convention) for callers that genuinely want "any one". `_toolStorageFurniture` Inspector field removed (was always null). The 2026-05-02 entry's `ToolStorage` "three-tier resolver" claim is superseded.

2026-05-02 — Farmer end-to-end rollout side-effect on logistics: `CommercialBuilding.GetToolStockItems()` is now a `virtual IEnumerable<ItemSO>` extension point used by `FindStorageFurnitureForItem` and `GoapAction_GatherStorageItems.DetermineStoragePosition` to route building-tool items (e.g. `FarmingBuilding.WateringCanItem`) to tool storage(s) and skip them for non-tools. `IsBuildingToolItem(ItemSO)` is the membership classifier. Tool-aware deposit flow detail: when a `JobLogisticsManager` worker drops off an item that returns `true` from `IsBuildingToolItem`, the deposit is redirected from the loose `StorageZone` into the role-tagged tool storage(s) (or the convention-resolved fallback) instead.
