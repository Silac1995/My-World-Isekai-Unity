...
- detaches the serialized `_content` TMP_Text up to the BuildingInspectorView root, then **hides every legacy prefab child** that isn't the header label or `_content` (the original prefab held `_content` deep inside a "Slots" wrapper with `LayoutElement(min=200, flex=10)` that would otherwise hog vertical space — see [[runtime-uichildren-collapse-under-prefab-vlg]]);
...
**Layout strategy: bypass the parent VLG via `ignoreLayout` + manual stretch anchors.** The prefab's outer `VerticalLayoutGroup` cannot reliably size the runtime-built sub-tab content hosts via `LayoutElement.flexibleHeight` — flex distribution misbehaves under mixed `flex=0` / `flex=1` siblings + a TMP-derived `Header` preferred height, leaving the active host clipped to ~30 px. The fix is to *not* fight VLG: each runtime host gets `LayoutElement.ignoreLayout = true` and a manual full-stretch RectTransform with a top inset for Header + SubTabBar. The canonical helper is `BuildingInspectorView.ConfigureContentHostStretch(host, topInset)`. Use the same pattern when adding a third sub-tab or any future runtime UI built atop a prefab-authored VLG. Don't try to tune `childForceExpandHeight` — it doesn't reliably solve the problem.
...
- **new host-only `DevForce*` server methods** that bypass owner / community-leader auth: `CommercialBuilding.DevForceSetHiring(bool)`, `CommercialBuilding.DevForceSetAssignmentWage(...)`, `CommercialBuilding.DevForceSetStorageRole(StorageFurniture, StorageRoleType)` (writes through `StorageFurnitureNetworkSync.SetRoleServer` — same NetworkVariable fan-out as the production `TrySetStorageRoleServerRpc`; honours `SupportedStorageRoles`), `CommercialBuilding.DevForceSetSafeRole(SafeFurniture, SafeRoleType)` (routes through the canonical `DoSetSafeRole` helper — same fan-out as the production `TrySetSafeRoleServerRpc` and the NPC `AssignSafeRolesForShift`; honours `SupportedSafeRoles`), `CommercialBuilding.DevForceChangeReputation(int delta)` *(2026-05-17f — fed by the new Reputation section's −10 / −1 / +1 / +10 nudge buttons; routes through `TryChangeReputation` so the standard clamp / `OnReputationChanged` event / replicated `_reputation` NetVar / audit log all fire identically to production rep events)*, `ShopBuilding.DevForceAddCatalogEntry / DevForceRemoveCatalogEntry / DevForceEditCatalogEntry / DevForceWithdrawCashierTill(Cashier, recipient)`.
...
- **Runtime UI children collapse under prefab `VerticalLayoutGroup`.** Full diagnosis + fix in [[runtime-uichildren-collapse-under-prefab-vlg]]. Short version: when you build new children at runtime under a GameObject whose prefab already has a `VerticalLayoutGroup`, (1) hide every legacy prefab child you don't explicitly need (their stale `LayoutElement.minHeight` / `flex` will hog space — `Destroy` is unreliable under `DontDestroyOnLoad`, use `SetActive(false)`), and (2) bypass the VLG entirely on the runtime hosts via `LayoutElement.ignoreLayout = true` + manual stretch anchors. `BuildingInspectorView.ConfigureContentHostStretch(host, topInset)` is the canonical helper — copy that pattern for any future runtime sub-tab build atop a prefab-authored VLG.
