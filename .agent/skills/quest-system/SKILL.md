...
- **`HandleClaimedListChanged` must handle `Remove` AND `RemoveAt`**: NGO's `NetworkList<T>` fires distinct event types — `Add`, `Insert`, `Value`, `Remove` (by-value), `RemoveAt` (by-index), `Clear`. `ServerTryAbandon` removes by index (`_claimedQuestIds.RemoveAt(i)`), so the event type on the wire is `RemoveAt`. The handler historically only checked `EventType.Remove`, which silently dropped the event on clients — the snapshot stayed in `_snapshots`, the HUD tracker never cleared, the marker stayed forever, and the Abandon button looked broken (it actually worked server-side, but the client view never updated). The handler now branches on both `Remove` / `RemoveAt` (single-id removal) and `Clear` (bulk wipe). See `CharacterEquipment.HandleEquipmentSyncListChanged` for the same pattern.
