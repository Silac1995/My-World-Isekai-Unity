...
- **[GoapAction_FulfillAmbitionConstruction](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_FulfillAmbitionConstruction.cs)** — composite action. Preconditions `ambitionBuildingFinalized=false`, effects `ambitionBuildingFinalized=true`, cost 5. `Execute()` runs a 5-step state machine, dispatching to whichever step is currently actionable: (1) carrying a relevant item → walk to BuildingZone + `CharacterDropItem`; (2) loose `WorldItem` matching a missing req nearby → walk + `CharacterPickUpItem`; (3) cached `_targetHarvestable` still yields a missing item + `CanHarvest()` → walk to its `InteractionZone` (rule #36 + softlock guard verbatim from `GoapAction_HarvestResources`) + `CharacterHarvestAction`; (4) scan `CharacterAwareness.GetVisibleInteractables<Harvestable>` for a new yielder; (5) inside zone with consumable items → `CharacterAction_FinishConstruction`. Step 6 (nothing actionable) sets `_isComplete=true` so the planner re-evaluates next throttle window and the BT falls through to Wander — the NPC drifts, awareness picks up new harvestables on the next position.
