...
## NeedAmbitionFinishConstruction + GoapAction_FulfillAmbitionConstruction (2026-05-18)

When a `CharacterAmbition` step contains a `Task_FinishConstruction` and the actor placed the matching `Building` themselves (canonical case: `Ambition_FoundACity` step 2 — Quest_BuildCapital), the autonomous gather + carry + drop + finalize loop is driven by a dedicated Need + composite GoapAction instead of being open-coded in a BTAction. This matches the `NeedJob` pattern: a Need provides an active goal + action list to `CharacterGoapController.Replan`, the planner picks it up, and the action shows up in the per-character GOAP inspector.

- **[NeedAmbitionFinishConstruction](../../Assets/Scripts/Character/CharacterNeeds/NeedAmbitionFinishConstruction.cs)** — `IsActive` walks the active `AmbitionQuest`'s tasks for any `Task_FinishConstruction` whose `TargetBlueprint` matches a still-under-construction `Building` placed by this actor (`PlacedByCharacterId == CharacterId`). `BASE_URGENCY = 75` (above `NeedJob`'s 60 so the founder commits to the capital first). `GetGoapGoal` → `{"ambitionBuildingFinalized" = true}`; `GetGoapActions` → `[GoapAction_FulfillAmbitionConstruction]`. Registered in `CharacterNeeds.Awake` alongside the other needs.

- **[GoapAction_FulfillAmbitionConstruction](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_FulfillAmbitionConstruction.cs)** — composite action. Preconditions `ambitionBuildingFinalized=false`, effects `ambitionBuildingFinalized=true`, cost 5. `Execute()` runs a layered state machine, dispatching to whichever step is currently actionable: **Step 0.5** consume-priority (inside zone + zone-resident WorldItem matches a missing req → `CharacterAction_FinishConstruction` BEFORE drop/pickup so a fresh drop gets eaten instead of re-picked); **Step 1** carrying a relevant item (HandsController OR bag inventory) AND `XZ-distance-to-zone-center ≤ min(1.5u, half-extent − 1u)` → `CharacterDropItem` (the gate stops the actor from dropping at the boundary where `ExecutePhysicalDrop`'s `±0.3u` random offset would land the item outside the collider); **Step 2** loose `WorldItem` matching a missing req within 12u that's NOT already inside the BuildingZone → walk + `CharacterPickUpItem` (`Physics.OverlapSphere` first, `FindObjectsByType` fallback for collider-lifecycle race after a fresh spawn); **Step 3** cached `_targetHarvestable` + `_targetMode` still classify the same way via `ClassifyYield` → walk to `InteractionZone` (rule #36 verbatim from `GoapAction_HarvestResources`) + queue `CharacterHarvestAction` (harvest mode) or `CharacterAction_DestroyHarvestable` (destroy mode); **Step 4** two-pass awareness scan via `CharacterAwareness.GetVisibleInteractables<Harvestable>` — pass 1 prefers `HasYieldOutput` (cheaper, preserves the node), pass 2 falls through to `HasAnyDestructionOutput` gated by `AllowDestruction + AllowNpcDestruction`; **Step 5** scene-wide `FindObjectsByType` scan (no distance cap) for the closest viable source — walks the actor TOWARD it so awareness picks up the source on approach (cost bounded by the 2s `CharacterGoapController.Replan` throttle); **Step 5b** active exploration wander at 36-60u via `NavMesh.SamplePosition` when the scene is truly empty of viable sources, kept inside the GoapAction so `CurrentAction` stays visible in inspectors instead of dropping to BT Wander. Walks-to-BuildingZone-center always project the Y coordinate to the actor's Y first (zone BoxColliders are tall enough that `bounds.center.y` exceeds NavMesh.SamplePosition's 5m sample tolerance).

Why composite (not 5 atomic GoapActions): keeps the new code footprint manageable while still rendering visibly in `CharacterGoapController.CurrentGoalName` / inspector tooling. Splitting into atoms (Harvest / Pickup / Walk / Drop / Consume as separate `GoapAction`s with chained pre/effects) is a future refactor; the canonical `CharacterAction` atoms are already reused underneath.

Network safety: GOAP runs server-only (`NPCBehaviourTree.Update` IsServer gate). All world mutations route through existing canonical `CharacterAction`s. No new replication channel introduced.

The matching `BTAction_PursueAmbition.TryDrive` returns `BTNodeStatus.Failure` for `Task_FinishConstruction` so the BT Selector moves on to GOAP — Ambition → **GOAP (this Need)** → Social → Wander. `Task_PlaceBuilding` and `Task_PromoteCommunity` stay driven directly by the BTAction (short, atomic operations that don't benefit from planner indirection).

## JobBuilder action library (Plan 4b, 2026-05-18)

Four NEW GoapActions wired into [JobBuilder](../../Assets/Scripts/World/Jobs/BuilderJobs/JobBuilder.cs); see `wiki/systems/administrative-building.md` Change log 2026-05-18 entry for the design + `docs/superpowers/plans/2026-05-18-job-builder-and-goap-actions.md` for the implementation plan.

- **[GoapAction_TakeMaterialFromABStorage](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_TakeMaterialFromABStorage.cs)** — Mirrors `GoapAction_FetchSeed` pattern verbatim. Walks every `StorageFurniture` in the AB's transform tree for any slot holding the next missing material from `_ab.LogisticsManager.GetFirstActiveBuildOrder()`. Movement gate = `IsCharacterInInteractionZone` (rule #36) + softlock guard + path-loss re-fire. Takes one instance via `storage.RemoveItem`, carries via `worker.CharacterEquipment.CarryItemInHand`. Cost = 1.0. Preconditions: `hasMaterialsInHand=false, hasActiveBuildOrder=true, hasMatchingMaterialInABStorage=true`. Effects: `hasMaterialsInHand=true`.

- **[GoapAction_GoToConstructionSite](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_GoToConstructionSite.cs)** — Pure move into `BuildOrder.TargetBuilding.BuildingZone.bounds`. The construction zone is a plain BoxCollider trigger (no `InteractableObject`) so arrival is 2D X-Z bounds containment, mirror of `CharacterAction_FinishConstruction.IsActorInsideBuildingZone`. Softlock guard + `!HasPath` re-fire. Cost = 0.5 (cheap). Preconditions: `hasMaterialsInHand=true, hasActiveBuildOrder=true`. Effects: `insideConstructionSite=true`.

- **[GoapAction_DropMaterialAtZone](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_DropMaterialAtZone.cs)** — Once inside the zone, queues `CharacterDropItem(worker, carried)`. The dropped instance becomes a loose `WorldItem` inside the zone, where `CharacterAction_FinishConstruction.ConsumeFromZone` despawns it the next tick. Uses the GatherStorageItems "wait pattern" — `_actionStarted` lock + `OnActionFinished` callback sets `_isComplete`. Cost = 0.5. Preconditions: `hasMaterialsInHand=true, insideConstructionSite=true`. Effects: `materialDelivered=true`.

- **[GoapAction_FinishBuildingConstruction](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_FinishBuildingConstruction.cs)** — Wraps `CharacterAction_FinishConstruction` (the Phase 1 continuous action). Subscribes to `OnActionFinished` for the wait. The continuous action exits on (a) progress hits 1.0 → `Building.Finalize`, (b) worker leaves zone, (c) MaxStallTicks of zero-consume. All three drop back to JobBuilder.PlanNextActions for the next trip. Cost = 1.0. Preconditions: `insideConstructionSite=true, materialDelivered=true`. Effects: `isIdling=true` (multi-trip handled by JobBuilder's force-replan on action completion).
