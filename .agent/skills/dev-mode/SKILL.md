...
- **CommercialBuilding ownership replication** — `CommercialBuilding.SetOwner` writes `_owner` server-side only; there is no `NetworkVariable` backing the field today (compare `ResidentialBuilding._ownerIds`). Dev-mode Assign-Building works on the host (the building's `Owner` getter returns the new character), but clients won't see the change until a proper network sync is added. Follow-up slice should migrate to a `NetworkVariable<FixedString64Bytes>`-backed ownerId pattern.
