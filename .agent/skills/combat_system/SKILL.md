...
- **CombatEngagementCoordinator**: A delegated pure C# class that manages brawl "subgroups" (`_activeEngagements`) using a **targeting-graph algorithm**. It maintains a `_targetingGraph` (`Dictionary<Character, Character>`) tracking who targets whom. Each battle tick, `EvaluateEngagements()` runs the full algorithm. Key rules:
  ...
  - **Spatial separation**: After every `EvaluateEngagements` tick, `SeparateEngagements()` runs pairwise repulsion. Engagement anchors closer than `MIN_ENGAGEMENT_SEPARATION = 12` units are pushed apart at `SEPARATION_PUSH_SPEED = 2` per tick. Perfectly overlapping anchors get a deterministic offset. Since drift positions are calculated relative to the anchor, separated anchors → separated character clusters.
...
4. **Idle standoff/drift**: Random angle + distance from focal point every **5-7 seconds**, up to **5 units**. Constrained within a standoff distance band: `min = 1`, `max = attackRange * 1.5 + 6` from opponent center. Uses **5-second throttle** for re-pathing.
   - **Angular slot spreading**: Each character's drift angle is anchored to a base slot derived from their index within their `EngagementGroup`. `GroupA` occupies the left semicircle (180° arc centered on π), `GroupB` occupies the right (centered on 0). Slots are evenly distributed across the arc with ±30° random variation per drift tick. This prevents allies on the same side from drifting onto the same spot.
