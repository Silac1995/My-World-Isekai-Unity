...
- `MapController.SpawnSavedBuildings()` respawns player-placed buildings AND overlays saved dynamic state onto already-in-scene buildings whose deterministic `BuildingId` matches a saved entry. Routed through `MapController.ApplyDynamicSaveDataToBuilding(Building, BuildingSaveData)` — the single helper used by both `SpawnSavedBuildings` (new spawn + preplaced overlay) and `WakeUp` (post-hibernation new spawn).
- **Restore order inside `ApplyDynamicSaveDataToBuilding` (load-bearing, 2026-05-13):** owners → employees → **`building.RestoreFromSaveData(bSave)` (state-flip + manual post-Complete cascade)** → placed-furniture → storage contents → cashier → shop hooks. Reordered 2026-05-13 — `RestoreFromSaveData` previously ran LAST, which meant `OnNetworkSpawn`'s auto-derived `UnderConstruction` state gated `TrySpawnDefaultFurniture` off, then the per-furniture content restore steps walked an empty furniture set and silently no-op'd, then `RestoreFromSaveData` flipped state to Complete but `HandleStateChanged` had no subscriber yet (wired in `Start`, which fires next frame) — so default furniture never spawned and saved storage contents vanished. Fix: `Building.RestoreFromSaveData` now manually invokes `TrySpawnDefaultFurniture()` + `ConfigureNavMeshObstacles()` inline when `_isStarted == false` AND state ends Complete; the call site reorder ensures the content-restore steps see a fully populated live furniture set. **Do not reorder this helper without re-reading [[save-restore-state-flip-no-subscriber]] in the wiki gotchas.**
