...
- **Deferred refactor (not in this fix):** `Cashier.ServerTickAutoOccupy` itself is architecturally wrong — vendors should occupy the cashier through a `CharacterAction` queued by `JobVendor` (and the player's E-press), gated by `InteractableObject.IsCharacterInInteractionZone`. Occupancy state should lock movement + other actions; leaving has its own `CharacterAction`; forced-leave (combat/damage) bypasses the action and goes through `AutoLeaveOccupiedFurniture` (already shipped). Until that refactor, NPCs still seat by proximity.
