...
## Change log
- 2026-05-18 — **Phase 2 NPC autonomy data + behavior layer SHIPPED** for the JobBuilder pipeline. Commits: BuildOrder IQuest + LogisticsOrderBook wiring (Tasks 1-2), four NEW GoapActions (TakeMaterialFromABStorage, GoToConstructionSite, DropMaterialAtZone, FinishBuildingConstruction — Task 3), JobBuilder class with goal cascade DeliverAndConstruct → FetchFromABStorage → Idle (Task 4), AdministrativeBuilding.InitializeJobs staffs city roster + unfulfillable-material harvest queue (Task 5), BuildingLogisticsManager.ProcessActiveBuildOrders cascade routes BuildOrder material demand through B2B → producer → virtual → AB-queue (Task 6). JobHarvester CityHarvester runtime branch (Task 7) is stubbed — logs queue contents and idles; the physical-harvest fallback execution is a follow-up. Pipeline ships fully functional for B2B + producer + virtual supplier paths; harvest fallback queues but does not yet drain. — claude
