...
| [BuildingConstructionOutline.cs](../../Assets/Scripts/World/Buildings/Construction/BuildingConstructionOutline.cs) | Peer-local cosmetic component on Building_prefab.prefab (inherited by every variant). Draws a flat yellow rectangle outline around `BuildingZone` while `UnderConstruction` via a single 4-vertex `LineRenderer` + shared runtime `Material(Shader.Find("Sprites/Default"))`. Event-driven (subscribes to `Building.OnConstructionStateChanged`), no Update, GameObject deactivated when not building. **Zero `.mat` assets on disk** — see [[material-buildproperties-standalone-crash]] for why this matters. |
| [BuildingConstructionOutlineSettings.cs](../../Assets/Scripts/World/Buildings/Construction/BuildingConstructionOutlineSettings.cs) | ScriptableObject tuning asset for the outline. **Only primitive fields** (Color, float, int, string) — zero `UnityEngine.Object` references → crash-safe in Resources/ (Rule #38). Asset lives at `Assets/Resources/Data/BuildingConstructionOutlineSettings.asset`. |
...
- `event Action<BuildingState, BuildingState> OnConstructionStateChanged` — fires on every peer (server + clients) from `HandleStateChanged` with `(previousValue, newValue)`. Cosmetic, peer-local subscribers only (outline renderer, scaffold hints, FX). Wrapped in try/catch per Rule #31 so a buggy subscriber can't block the rest of the state-change cascade. **Do not put server-authoritative side effects here** — those belong directly in `HandleStateChanged`.
...
- **Multi-treasury-safe ordering for BaseTreasury seed** — `OnDefaultFurnitureSpawned` is now also the entry-point for `CommercialBuilding.SeedTreasuryIfNeeded()`, which credits `BuildingCommercialSO.BaseTreasury` into the Treasury-role safe via [[commercial-treasury|CreditTreasury]]. Currently delegates to `CreditTreasury`'s safe-selection logic (largest-safe-first across `TreasurySafes`). If a building authors multiple Treasury-role safes in the future, revisit which safe receives the seed (open question deferred from Task 9). See [[commercial-building#Treasury seed flow]] for the full data flow.
- **The renderer-sweep gotcha (2026-05-18c).** `Building.EnsureConstructionGhostVisual` (called once from `Building.Start`) walks every top-level child of the Building EXCEPT `_constructionVisualRoot`, `_completedVisualRoot`, and any GameObject carrying a `Zone` component, and caches every `Renderer` it finds into `_extraOriginalRenderersToToggle`. `Building.ApplyConstructionVisuals` then sets `r.enabled = !underConstruction` on every cached renderer — purpose is to hide things like the Interior Door's wall/archway during construction. **`LineRenderer`, `MeshRenderer`, `SkinnedMeshRenderer`, `ParticleSystemRenderer`, `TrailRenderer`, `SpriteRenderer`, … all inherit from `Renderer`** and therefore get swept by this iteration. If you add a component to a Building (or its base prefab) that creates a Renderer-derived child intended to be VISIBLE during construction, you MUST do one of:
  1. **`[DefaultExecutionOrder(N)]` with N > 0 on your component** so its `Start` runs after `Building.Start`. The cache snapshot is built BEFORE your Renderer-bearing child exists → never cached → never disabled. This is the canonical fix; see `BuildingConstructionOutline.cs`.
  2. **Parent your Renderer GO under `_constructionVisualRoot` or `_completedVisualRoot`.** Those two roots are skipped by the iteration (line 630-631). Caveat: `EnsureConstructionGhostVisual` wipes existing children of `_constructionVisualRoot` first (line 615-619), so order-dependent.
  3. **Parent under a `Zone`-bearing GameObject** (e.g., a `DeliveryZone` / `StorageZone` child). Skipped at line 632. Hacky — only use if option 1 is impossible.
  The naive "just set `r.enabled = true` in OnEnable" does NOT work because `ApplyConstructionVisuals` runs INSIDE `HandleStateChanged` which runs INSIDE `Building.Start` and on every subsequent state change — your enable would be overridden by the next sweep.
...
- [~] **NPC owner autonomy** — Phase 2 design landed 2026-05-18 as part of the City Founding spec; NPC autonomy now lands as `JobBuilder` employees of the new `AdministrativeBuilding` (NOT free-time citizen helping — that design was explicitly rejected; citizens have their own lives). Awaiting implementation plan. See [docs/superpowers/specs/2026-05-18-city-founding-and-administrative-building-design.md](../../docs/superpowers/specs/2026-05-18-city-founding-and-administrative-building-design.md).
- [~] **Community-manager city-management console** — spec'd 2026-05-18 as `CityManagementFurniture` + `UI_CityManagementPanel` inside the AdministrativeBuilding. Includes RTS-style city blueprint placement, BuildOrder management, join-request processing, tier promotion. Same spec link as above.
- [~] **`JobBuilder` GOAP job class** — spec'd 2026-05-18, hosted on `AdministrativeBuilding`, consumes new `BuildOrder` (a logistics order parallel to `BuyOrder`/`CraftingOrder`/`TransportOrder`). Materials cascade: shop → CraftingBuilding → VirtualResourceSupplier → fallback to AB `JobHarvester`. Same spec link as above.
...
## Change log
- 2026-05-18c — **Construction-zone outline finalised + cooperative-placement-always-engages-UnderConstruction.**
  * **`Building.OnNetworkSpawn` runtime-placed branch**: any building whose `PlacedByCharacterId` is non-empty now starts in `UnderConstruction` regardless of `EffectiveConstructionRequirements.Count`. Scene-authored buildings keep the legacy reqs==0 → Complete shortcut. `_spawnAsComplete` still takes precedence; save/load still overrides. This finally honours the long-documented `_spawnAsComplete` tooltip intent ("runtime placements via BuildingPlacementManager always go through UnderConstruction → Complete unless InstantMode is on").
  * **`BuildingPlacementManager.RequestPlacementServerRpc`**: calls `BuildInstantly()` when `instant=true` OR `reqCount == 0`. The 0-req branch is what was missing — without it, a 0-req placement (Clothing Shop, Transport, AdministrativeBuilding blueprints) would be stuck in `UnderConstruction` forever now that OnNetworkSpawn no longer auto-completes them. `BuildInstantly`'s deferral coroutine (waits for `Building.Start` to set `_isStarted`) provides the few-frame "delay of some milliseconds" window for the outline / scaffold ghost / curtain to render before the state flips to Complete.
  * **`Building.EnsureConstructionFootprintMarker` Y-clamp**: applied `Mathf.Max(boxBottomLocalY, 0f) + 0.02f` instead of the raw `box.center.y - size.y/2 + 0.02f`. Lumberyard's BoxCollider is authored with bottom face at local Y = −0.43 (extends below building pivot); the unclamped formula placed the translucent footprint Quad at world Y = −0.41, hidden underground. Same bug class as the one already fixed in `BuildingConstructionOutline.EnsureOutlineCreated` (2026-05-18b). Going forward, any visual placed at the bottom face of `BuildingZone` MUST use the Y-clamp.
  * **`BuildingConstructionOutline` execution order fix**: added `[DefaultExecutionOrder(100)]` so the outline's `Start` runs AFTER `Building.Start`. `Building.EnsureConstructionGhostVisual` (called from Start) walks every top-level child Renderer and caches them into `_extraOriginalRenderersToToggle`; `ApplyConstructionVisuals` then disables every cached renderer while UnderConstruction. Since `LineRenderer` inherits from `Renderer`, the outline's LineRenderer was being swept into the cache and disabled — exactly the opposite of what was wanted. With the ordering fix, the outline GO is created AFTER the cache snapshot is taken, so it is never cached and never disabled. New gotcha entry added below.
  * **Default outline width / height**: bumped Width 0.15 → 0.5 and HeightOffset 0.05 → 0.15 in `BuildingConstructionOutlineSettings.asset`. 0.15u was sub-pixel at typical isometric camera distances; 0.5u (~7.5 cm in world scale) reads clearly. The extra height keeps the line cleanly above the cyan footprint Quad (which sits at Y = 0.02).
  * **`OnConstructionStateChanged` already present from 2026-05-18b**; no API change. — claude / [[kevin]]
- 2026-05-18b — **Construction-zone outline shipped.** New peer-local cosmetic component `BuildingConstructionOutline` + shared SO `BuildingConstructionOutlineSettings` draw a yellow rectangular outline (LineRenderer, 4 verts, `loop = true`) around the `BuildingZone` while `UnderConstruction`. Added one event `Building.OnConstructionStateChanged(prev, new)` fired from `HandleStateChanged` so the outline subscribes without coupling Building.cs to the cosmetic layer. Component lives on `Assets/Prefabs/Building/Building_prefab.prefab` (the base) so every variant inherits the wired SO automatically — mirrors the `BuildingCurtainSettingsHolder` pattern. **Crash-safe by construction** (see [[material-buildproperties-standalone-crash]]): zero new `.mat` assets, zero new shaders, zero texture refs. The runtime Material is built via `new Material(Shader.Find("Sprites/Default"))` with `HideFlags.HideAndDontSave` — never serialized. The SO has only primitive fields (Color, float, int, string) — zero `UnityEngine.Object` references, safe to live in Resources/. One shared static Material across all outlines → 1 draw call regardless of how many sites are active. Network behaviour: not a NetworkBehaviour; each peer renders its own LineRenderer locally driven by the replicated `_currentState`. — claude / [[kevin]]
- 2026-05-18 — **Phase 2 NPC autonomy data + behavior layer SHIPPED** for the JobBuilder pipeline. Commits: BuildOrder IQuest + LogisticsOrderBook wiring (Tasks 1-2), four NEW GoapActions (TakeMaterialFromABStorage, GoToConstructionSite, DropMaterialAtZone, FinishBuildingConstruction — Task 3), JobBuilder class with goal cascade DeliverAndConstruct → FetchFromABStorage → Idle (Task 4), AdministrativeBuilding.InitializeJobs staffs city roster + unfulfillable-material harvest queue (Task 5), BuildingLogisticsManager.ProcessActiveBuildOrders cascade routes BuildOrder material demand through B2B → producer → virtual → AB-queue (Task 6), and JobHarvester CityHarvester 7-state state machine drains the AB queue via Physics.OverlapSphere scan + CharacterHarvestAction + CharacterPickUpItem + CharacterStoreInFurnitureAction (Task 7). End-to-end pipeline live: BuildOrder placed → JobLogisticsManager sources via B2B/producer/virtual/harvest → JobBuilder fetches from AB storage → walks to site → drops → CharacterAction_FinishConstruction consumes + advances ConstructionProgress → Building.Finalize on completion. — claude
- 2026-05-18 — **Phase 2 NPC autonomy design landed** (`[[city-founding-and-administrative-building]]` spec). NPC autonomy is delivered via a hired `JobBuilder` employed at a new `AdministrativeBuilding : CommercialBuilding` consuming a new `BuildOrder` logistics order type — NOT via the originally-proposed free-time citizen helping (rejected; citizens have their own jobs). `BuildOrder` cascades through the existing supply chain (B2B shops → CraftingBuilding → VirtualResourceSupplier) with a final fallback to AB-hosted `JobHarvester` using `CharacterAwareness` to find nearby `Harvestable`s. Leader can opt-in to any step (Rule #22 parity via the existing cooperative Phase 1 loop — no new code for leader participation). The three Phase 2 Open Questions ("NPC owner autonomy", "Community-manager console", "`JobBuilder` GOAP job class") are now spec'd and awaiting implementation plan. — claude
