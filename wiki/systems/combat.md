...
- 2026-05-17 (gameplay cleanup) — **Removed the "ranged weapon falls back to melee at close range" branch in `CharacterCombat.ExecuteAttackLocally`.** Previous behavior: a player carrying a bow/pistol who was within `RangedCombatStyleSO.MeleeRange` (3.5m default) would auto-swing a melee attack on Attack input. New behavior: ranged weapons fire ONLY ranged attacks regardless of distance; if no target is set, the attack is a no-op. To melee while carrying a ranged weapon, the player must Swap to a melee weapon first (Y hotkey or Swap button on the combat action bar). Aligns gameplay with the combat-bar's active-weapon-only UI (spec [[combat-action-bar-design]] §3 decision #2). `MaxSharpness` field on `WeaponSO` is left as-is — only `MeleeWeaponInstance` consumes it, ranged instances ignore the field entirely, so it's dead data on ranged-WeaponSO assets but not actively harmful.
