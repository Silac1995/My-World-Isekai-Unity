...
- **State-flip writes in restore code don't fire `OnValueChanged` subscribers wired in `Start`.** When `MapController.SpawnSavedBuildings` → `Spawn()` → `ApplyDynamicSaveDataToBuilding` → `Building.RestoreFromSaveData` runs synchronously in one frame, `Start()` hasn't dispatched yet, so any subscriber wired there (`_currentState.OnValueChanged += HandleStateChanged`) is missing when the state flip fires. The post-Complete cascade silently no-ops. Two fixes are valid for this race depending on the caller's tolerance for a one-frame delay: **manual-cascade-inside-restore** (used by save-restore — the per-furniture content-restore steps need furniture live SAME-frame) and **coroutine-defer** (used by `BuildInstantly` — its gameplay path can tolerate one frame). See [[save-restore-state-flip-no-subscriber]] and [[buildinstantly-pre-start-lifecycle-race]].
- **Building restore ordering invariant.** `MapController.ApplyDynamicSaveDataToBuilding` must call `Building.RestoreFromSaveData` BEFORE the per-furniture restore steps (`RestorePlacedFurnitureForBuilding`, `RestoreStorageFurnitureContents`, `RestoreCashierContents`, ShopBuilding hooks). The state-flip inside `RestoreFromSaveData` is what triggers the manual post-Complete cascade that spawns default furniture; running content-restore before that fires walks an empty live-furniture set and silently drops every saved item. See [[save-restore-state-flip-no-subscriber]] for the full rationale.
- **Unity-serialized self-cycle on a save DTO crashes the standalone build during load** (editor only warns). A `[Serializable]` class with a field of its own type (directly or via `List<>` / array) creates a type-graph recursion that Unity's reflection serializer walks eagerly whenever a `[SerializeField]` chain reaches it. Hits the depth-10 limit on every scene scan: yellow warning in editor, native stack-overflow crash in the Mono player. Fix the cycle, do not just silence the warning. For DTOs round-tripped by Newtonsoft.Json (project default — `SaveFileHandler` uses no custom settings), `[System.NonSerialized]` on the offending field is the right tool: Newtonsoft ignores it (`IgnoreSerializableAttribute = true` by default) so JSON persistence is preserved, while Unity stops walking the type. The 2026-05-15 instance: `CharacterProfileSaveData.partyMembers` (`List<CharacterProfileSaveData>`) reached via `WildernessZone._wildlife` → `HibernatedNPCData.ProfileData`. See [[unity-serializer-self-cycle-build-crash]] for the full diagnostic recipe + decision tree (`[NonSerialized]` vs `[SerializeReference]` vs id-by-reference).
...
## Change log
- 2026-05-15 — Fixed: standalone build crashed natively during the loading sequence after the user clicked a character in `CharacterSelectPanel` (editor was fine). Root cause: `CharacterProfileSaveData.partyMembers` was `public List<CharacterProfileSaveData>` — a self-referencing type. Unity's reflection-based serializer eagerly walked the cycle every time the type was reached through a `[SerializeField]` chain (`WildernessZone._wildlife : List<HibernatedNPCData>` → `HibernatedNPCData.ProfileData : CharacterProfileSaveData` → `partyMembers : List<CharacterProfileSaveData>` → …). Editor hit Unity's depth-10 limit and just logged the standard `"Serialization depth limit 10 exceeded"` warning; the Mono standalone build overflowed the native stack during scene load and the process died with only a `SymGetSymFromAddr64` / `RtlUserThreadStart` tail in `Player.log` — no managed exception. Fix: added `[System.NonSerialized]` to `partyMembers`. Unity now skips the field (no more depth warning, no more native crash). Newtonsoft.Json (used by `SaveFileHandler` with default settings — `IgnoreSerializableAttribute = true`) continues to round-trip the field, so party persistence is unchanged and existing `Profiles/{characterGuid}.json` saves are byte-compatible. Audit of `Assets/Scripts/` confirmed no other `[Serializable]` types have unprotected self-cycles. Full diagnostic recipe + decision tree (`[NonSerialized]` vs `[SerializeReference]` vs id-by-reference) captured in new gotcha [[unity-serializer-self-cycle-build-crash]]. — claude
- 2026-05-13 (later) — Fixed the second half of the building-restore storage bug: **prefab-authored default furniture** was also vanishing, not just player-placed. Same `OnValueChanged`-with-no-subscriber race as `BuildInstantly` (see [[buildinstantly-pre-start-lifecycle-race]]) — `MapController.SpawnSavedBuildings` → `Spawn()` → `OnNetworkSpawn` auto-derives `_currentState.Value = UnderConstruction` for any prefab with `_constructionRequirements.Count > 0` and `_spawnAsComplete=false` (gates `TrySpawnDefaultFurniture` on that initial state). Then `ApplyDynamicSaveDataToBuilding → RestoreFromSaveData` flips `_currentState.Value = Complete`, **but `Start()` (where the `OnValueChanged += HandleStateChanged` subscription lives) hasn't run yet** — synchronous spawn-and-restore happens in one frame; `Start` is next frame. No subscriber → `HandleStateChanged` never runs → no default-furniture spawn, no NavMesh carve, and the per-storage content-restore step then walks an empty live-furniture set and silently no-ops. Fix: `Building.RestoreFromSaveData` now manually invokes `TrySpawnDefaultFurniture()` + `ConfigureNavMeshObstacles()` after the state-flip when `_isStarted == false`; `MapController.ApplyDynamicSaveDataToBuilding` reordered so `RestoreFromSaveData` runs BEFORE the per-furniture restore steps (so content-restore sees live storages). Save-restore can't use the BuildInstantly coroutine-defer pattern because the content-restore steps need furniture live in the same synchronous call, not next frame. See new gotcha [[save-restore-state-flip-no-subscriber]] for the full pattern + alternative-considered rationale. — claude
- 2026-05-13 — Fixed: player/NPC-placed furniture (chests, crafting stations, etc.) now persists across save/load on every Building subclass. Pre-fix, `BuildingSaveData` only saved storage CONTENTS (`StorageFurnitures` slot snapshots keyed by furniture position) but never the furniture's EXISTENCE — on load, the building re-spawned from prefab and only `_defaultFurnitureLayout` slots re-instantiated. ShopBuilding appeared to work because its SellShelves were nested-NO prefab children absorbed into `_defaultFurnitureLayout` at Awake by `ConvertNestedNetworkFurnitureToLayout`; HarvestingBuilding / CraftingBuilding / FarmingBuilding storages were typically player-placed and silently vanished. Fix: added `BuildingSaveData.PlacedFurnitures : List<PlacedFurnitureSaveEntry>` + new `Building.IsDefaultLayoutFurniture` dedup helper + new `MapController.RestorePlacedFurnitureForBuilding` server-only restore step that runs BEFORE `RestoreStorageFurnitureContents` (so per-storage key lookup finds the freshly-spawned instances). Both refresh paths (`SnapshotActiveBuildings`, `Hibernate`) carry the new field. Same refresh-path trap that bit `ConstructionProgress` / `DeliveredMaterials` on 2026-05-07. Backward-compatible: empty list defaults so old saves deserialize cleanly. See [[building]] for the full schema. — claude
- 2026-05-09 (later) — Fixed: building owners now persist across save/load for the **host's player Character** too (NPCs already worked). The bug: GameLauncher spawns the host's player Character at Step 4 with a fresh `Guid.NewGuid()`, then `SpawnSavedBuildings` runs at Step 5b — `Building.RestoreOwnersFromSaveData` calls `Character.FindByUUID(savedHostGuid)` and finds nothing because the host's persistent GUID hasn't been imported yet. The pending list subscribes to `Character.OnCharacterSpawned`, but that event already fired in Step 4 with the WRONG GUID. Step 6 imports the profile and overwrites `NetworkCharacterId.Value` to the saved GUID — too late. Fix: added new static event `Character.OnCharacterIdReassigned`, fired from `CharacterDataCoordinator.ImportProfile` when `NetworkCharacterId.Value` actually changes; both `Building.RestoreOwnersFromSaveData` and `CommercialBuilding.RestoreEmployeesFromSaveData` now subscribe to BOTH events. NPCs continue to resolve via `OnCharacterSpawned` (their UUID is set inside `SpawnNPCsFromSnapshot → ImportProfile` BEFORE OnNetworkSpawn). The host's player resolves via `OnCharacterIdReassigned` after profile import. (Player jobs were already working because `CharacterJob.Deserialize` runs on the character side from `ImportProfile` itself and binds via BuildingId — that's why the user observed jobs persisting but ownership lost.) — claude
- 2026-05-09 — Fixed: building owners now persist across save/load for **every** Building subclass — not just `CommercialBuilding`. The pending+`OnCharacterSpawned` resolver lifted to base `Building` (`RestoreOwnersFromSaveData` + virtual `BindRestoredOwner` hook); `ResidentialBuilding` overrides the hook to call its `SetOwner` (residency mirror), `CommercialBuilding` overrides it to call `SetOwner(autoAssignJob:false)` + tie back to the matching saved `EmployeeSaveEntry` for the boss's actual job slot. Previously the load path was a single `if (restoredBuilding is CommercialBuilding commercial) commercial.RestoreFromSaveData(...)` — every other Building subclass dropped its `OwnerCharacterIds` silently. Same fix removed `SnapshotActiveBuildings`'s `if (PlacedByCharacterId.IsEmpty) continue` filter that was dropping all dynamic-state mutations on scene-authored buildings, and replaced `SpawnSavedBuildings`'s "skip if already in scene" branch with an overlay branch (`ApplyDynamicSaveDataToBuilding`) so preplaced buildings get owners/employees/storage/cashier/shop catalog applied to the live scene instance. See [[building]] for the BindRestoredOwner override matrix. — claude
