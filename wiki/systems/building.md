...
- **NEVER put `Building` / `FurnitureGrid` / `FurnitureManager` / `BuildingInteractable` / `ConstructionSiteScanner` / `BuildingTaskManager` / `BuildingLogisticsManager` on the root `Building_prefab.prefab` OR on a Tier-1 type base** (`CommercialBuilding_prefab.prefab`, `House prefab.prefab`). The `Building → ComplexRoom → Room → Zone` chain plus `[RequireComponent(typeof(Building))]` on `BuildingInteractable.cs:20` and `ConstructionSiteScanner.cs:16` means Unity auto-adds a base `Building` to whatever parent prefab carries those scripts; every Tier-2 variant (which carries its own `CommercialBuilding`/`ShopBuilding`/`ForgeBuilding`/etc. subclass) then inherits a second `Building`-chain `NetworkBehaviour`, and `Building.Awake → ConvertNestedNetworkFurnitureToLayout` runs twice, destroying nested children twice, crashing the Editor natively on the second pass. These scripts belong on the Tier-2 specific variant — the duplication across variants is correct given the polymorphic shape. Full diagnosis + recovery recipe at [[duplicate-building-component-on-prefab-variant-root]].
...
## Change log
- 2026-05-19 — Captured the **duplicate-Building-on-prefab-variant-root** failure mode in a new gotcha page [[duplicate-building-component-on-prefab-variant-root]] and cross-referenced it from the Known gotchas section. Triggered by Kevin's 2026-05-19 attempt to lift `Building` + `FurnitureGrid` + `FurnitureManager` + `BuildingInteractable` + `ConstructionSiteScanner` up to the root `Building_prefab.prefab` and the Tier-1 `CommercialBuilding_prefab.prefab`. `[RequireComponent(typeof(Building))]` on `BuildingInteractable` + `ConstructionSiteScanner` makes Unity auto-add a base `Building` to whatever parent prefab carries those scripts → every Tier-2 variant inherits two `Building`-chain `NetworkBehaviour`s (the inherited base + its own `ShopBuilding`/`ForgeBuilding`/`CommercialBuilding` subclass) → `Building.Awake → ConvertNestedNetworkFurnitureToLayout` runs twice → native Editor crash on the second pass. The lift is **structurally impossible** without first removing those `[RequireComponent(typeof(Building))]` attributes. Per-variant duplication of these scripts is the correct shape. Root + Tier-1 type bases must stay clean of the Building chain. Recovery recipe (revert root + Tier-1, save patch snapshot, idempotently re-add per-variant scripts via Roslyn `AddComponent`, verify counts vs HEAD) documented in the gotcha. — claude
