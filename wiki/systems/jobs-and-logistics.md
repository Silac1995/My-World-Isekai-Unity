...
## Change log
- 2026-05-20 — Harvester wrong-pickup fix (Part 2 — regression revert). The Part 1 fix below initially added a `taskMgr.HasAvailableOrClaimedTask<PickupLooseItemTask>` check to `GoapAction_PickupLooseItem.IsValid` (intending defense-in-depth). This rejected Pickup at punch-in time (no items dropped yet) — planner could not form `Harvest/Destroy → Pickup → Deposit` because Pickup was filtered out of `_scratchValidActions`; null plan; worker stuck on `Planning / Idle`. Reverted: the accepted-item filter lives ONLY in `Execute.ClaimBestTask` (Execute-time selection, runs after the chain produced the item). Layer 1 (registration) and Layer 3 (worldState symmetry) unchanged. The textbook violation of [[chain-action-isvalid-pre-filter]] is now cross-linked from both gotcha docs. — claude
- 2026-05-20 — Harvester wrong-pickup fix. `CharacterActions.ApplyHarvestOnServer` + `ApplyDestroyOnServer` registered a `PickupLooseItemTask` for EVERY spawned `WorldItem`, with no filter on whether the workplace accepted it. Destroying an apple tree (drops wood + apple sapling) at a wood-only lumberyard left both tasks live; `ClaimBestTask` could return the sapling first; worker carried the sapling, `hasAtLeastOneResource` stayed false (sapling not in `acceptedItems`), no deposit chain formed, worker froze. Three-layer fix: (1) `CharacterActions` gates registration on `HarvestingBuilding.GetAcceptedItems().Contains(spawned.ItemSO)`; (2) `GoapAction_PickupLooseItem.IsValid` + `Execute.ClaimBestTask` predicate filters by accepted items (defense in depth); (3) `JobHarvester.PlanNextActions` `looseItemExists` predicate mirrors the action's filter (canonical worldState/IsValid symmetry). Non-wanted byproducts (saplings, etc.) now stay on the ground for players. New gotcha [[harvester-picks-wrong-loose-item]]. — claude
- 2026-05-19 — `JobHarvester.PlanNextActions` deposit-freeze fix. The "lie to the planner" trick (set `hasResources=false` while bag has free space, so the planner keeps chaining `Harvest → Pickup`) was unconditional on whether more harvest work was actually claimable. Result: a harvester who destroyed the only available tree, picked up the wood, then re-planned was left with `hasHarvestZone=false`, `looseItemExists=false`, `hasResources=false` (lie) → no plan extension to `hasDepositedResources=true` → null plan → worker frozen mid-zone holding wood. Fix moves the task-availability scan above the `hasResourcesForGoap` computation and adds `(canHarvest || looseItemExists)` to the continue-gather gate. When nothing more is actionable, the lie flips off and the planner picks `DepositResources` instead of stalling. Mirrors FarmingBuilding's worldState/IsValid symmetry canonical pattern (2026-05-02). New gotcha [[harvester-deposit-freeze]]. — claude
- 2026-04-30 — JobFarmer added to JobCategory.Harvester family. PlantCropTask + WaterCropTask quest-eligibility wired to JobType.Farmer in `CommercialBuilding.DoesJobTypeAcceptQuest`. Plan 3 of the 4-plan Farmer rollout (Plan 1 Tool Storage + Plan 2 Help Wanted + Plan 2.5 Management Furniture + Plan 3 Farmer integration). See [[job-farmer]]. — claude
