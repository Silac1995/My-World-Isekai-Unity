...
| [CharacterInspectorView.cs](../../Assets/Scripts/Debug/DevMode/Inspect/CharacterInspectorView.cs) | `IInspectorView` for `CharacterInteractable`. Owns 10 `SubTabEntry` references; `Update()` refreshes only the active sub-tab. Also hosts 3 dev-mode copy buttons (`_copyUidButton` / `_copyWorldGuidButton` / `_copyAllButton`) in a sibling `IdentityCopyRow` that put the inspected `Character.CharacterId` (the persistent UID for save lookups), `Character.OriginWorldGuid` (the [[character-profile]] origin world), or both as a `Name + UID + OriginWorld` block onto `GUIUtility.systemCopyBuffer`. Buttons auto-disable when no character is selected. |
...
- 2026-05-09 — Added 3 dev-mode copy buttons on `CharacterInspectorView` for fast clipboard capture of identity strings used by [[save-load]] / [[character-profile]] lookups. New serialized fields `_copyUidButton` / `_copyWorldGuidButton` / `_copyAllButton`; click handlers write to `GUIUtility.systemCopyBuffer` (UID = `Character.CharacterId`; world = `Character.OriginWorldGuid`; all = a `Name + UID + OriginWorld` block) wrapped in `try/catch` per rule 31, with a single `Debug.Log` per click. Buttons live in a new `IdentityCopyRow` GameObject (sibling index 1, between `Header` and `TabBar`) with `HorizontalLayoutGroup(spacing=6, padding=4/4/2/2, MiddleCenter, ChildControl=true, ChildForceExpand=true)` + `LayoutElement(preferredHeight=28)`; each button has `Image + Button + LayoutElement(minWidth=80, flexibleWidth=1, preferredHeight=24)` and a stretching child `Label` (TMP_Text, fontSize=12, color=0.9 grey, `TextAlignmentOptions.Center`). `SetTarget` / `Clear` toggle `Button.interactable` so buttons grey-out when no character is selected. Wired into the `DevModePanel.prefab` via `script-execute` + `SerializedObject.FindProperty` (no Editor builder needed for 3 buttons). — claude
