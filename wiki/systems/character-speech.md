...
- Prefabs: [SpeechBubbleInstance_Prefab.prefab](../../Assets/Prefabs/SpeechBubbleInstance_Prefab.prefab) (the bubble UI — `RectTransform` + `CanvasGroup` + Image bg + `Text_Speech` TMP; the 2026-05-19 rework adds `NameStripBackground` + `NameText` + `TailRoot` children and drops the old `SeparatorLine` — script-side already lands in Task 9, prefab restructure pending Task 8 once the Editor is on this worktree), [UI_PlayerHUD.prefab](../../Assets/UI/Player%20HUD/UI_PlayerHUD.prefab) (hosts `HUDSpeechBubbleLayer` + `ContentRoot`).
