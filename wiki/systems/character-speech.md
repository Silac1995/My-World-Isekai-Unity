...
- Prefabs: [SpeechBubbleInstance_Prefab.prefab](../../Assets/Prefabs/SpeechBubbleInstance_Prefab.prefab) (the bubble UI — `RectTransform` + `CanvasGroup` + `NameStrip` (`Image` accent + `Text_Name` TMP) + `BodyPanel` (translucent `Image` + `Text_Speech` TMP + `Tail` image). No `VerticalLayoutGroup` / `ContentSizeFitter` on the root — sizeDelta is driven by `SpeechBubbleInstance.ApplyAdaptiveSize`; see [[runtime-uichildren-collapse-under-prefab-vlg]] for why the layout-group approach was abandoned.), [UI_PlayerHUD.prefab](../../Assets/UI/Player%20HUD/UI_PlayerHUD.prefab) (hosts `HUDSpeechBubbleLayer` + `ContentRoot`).
...
- **`UnityEngine.Color` cannot live directly on a Newtonsoft-serialised save struct.** The 2026-05-20 surfacing: `CharacterProfileSaveData.accentColorOverride` started as `Color`, which Newtonsoft.Json walked into the `linear → gamma → linear …` property loop and exception'd out, breaking every save. Fixed by switching the save field to `Color32` and casting at the boundary. Future colour-on-save fields must follow the same convention. Details: [[newtonsoft-color-self-referencing-loop]].
...
## Change log
- 2026-05-20 — Save round-trip fix: `CharacterProfileSaveData.accentColorOverride` swapped `Color → Color32` to dodge Newtonsoft.Json's self-referencing loop on `UnityEngine.Color.linear` / `.gamma`. Every character now serialises an override since the deterministic-random spawn change, so this was load-bearing. See [[newtonsoft-color-self-referencing-loop]]. — claude
- 2026-05-20 — Random per-character accent on spawn: FNV-1a hash of `CharacterId` → HSV (random hue, sat 0.55, val 0.75) replaces the archetype-default seed on `OnNetworkSpawn`. Persisted via the existing `_hasAccentOverride` save path so the colour is stable across save/load. — claude
- 2026-05-20 — Frame-0 sizing: `SpeechBubbleInstance` extracts `PrepareTextAndApplySize` (text + `ForceMeshUpdate` + `ApplyAdaptiveSize`) and calls it synchronously from `Setup`/`SetupScripted` before the entrance animation, so the bubble fades in at its final size instead of flashing the prefab's authored default for ~0.3s. — claude
- 2026-05-20 — Bubble auto-sizes to the message (`SpeechBubbleInstance.ApplyAdaptiveSize`): width = clamp(text + padding, `_minBubbleWidth` 180, `_maxBubbleWidth` 320); past max, the body wraps and the bubble grows taller. Driven by `TMP_Text.GetPreferredValues`, applied once per message inside `TypeMessage` so the bubble stays stable during the typewriter reveal. — claude
- 2026-05-20 — Prefab restructure fix: dropped the `VerticalLayoutGroup` + `ContentSizeFitter` that an earlier Task 8 commit added to the root. With the stretched-anchor children carrying no `LayoutElement`-declared preferred size, the CSF collapsed the root height to 0 and child widths followed, rendering text vertically (one character per line). Layout now uses direct anchors: NameStrip top-stretched, BodyPanel full-stretched with `offsetMax.y = -22`. Mirrors the pattern documented in [[runtime-uichildren-collapse-under-prefab-vlg]]. — claude
