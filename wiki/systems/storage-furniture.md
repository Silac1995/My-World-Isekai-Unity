...
- **Interaction point is mandatory in practice.** `Furniture.GetInteractionPosition()` falls back to `transform.position` when `_interactionPoint` is null, which sits inside the base `NavMeshObstacle` carve. Workers can't path inside the carve. Mitigations in priority order:
  1. **Zone-membership arrival (2026-05-14):** `GoapAction_GatherStorageItems` and `GoapAction_TakeFromSourceFurniture` now treat `InteractableObject.IsCharacterInInteractionZone(worker)` as the canonical "arrived" check (Interactable System Core Rule #1), with an arrived-but-just-outside-2f softlock fallback. This means the worker only needs to navigate **inside the InteractionZone** — not to the precise `_interactionPoint`. Mirrors the canonical pattern already in `GoapAction_FetchToolFromStorage` and `GoapAction_ReturnToolToStorage`. The legacy 1.5f flat-XZ check is retained only for prefabs with no `FurnitureInteractable.InteractionZone` at all.
  2. **`Furniture.Awake` runtime safety net (2026-05-14):** if `_interactionPoint` is null AND no `FurnitureInteractable.InteractionZone` is configured, `Awake` auto-spawns the `InteractionPoint` child by re-using `AutoCreateInteractionPoint()`. Closes the hole for legacy prefabs and runtime-placed furniture (placement / save-restore) that bypass the editor `Reset()` hook. Local-only (no network sync needed — both peers run Awake on identical renderer geometry).
  3. The auto-create-on-Reset hook adds an `InteractionPoint` child for new prefabs.
  4. The `[ContextMenu]` "Auto Create Interaction Point" action regenerates it for existing prefabs.
  5. The `GetInteractionPosition(Vector3 fromPosition)` overload uses `InteractionZone.bounds.ClosestPoint` as the fallback when no point is authored but a `FurnitureInteractable` is — typically lands on a navmesh-walkable face.
  6. As a last resort, both deposit/withdraw actions still carry a 5-second softlock guard that blacklists the furniture and falls back to the loose path. The orange `[GOAP Storage]` warning emitted inside `GoapAction_GatherStorageItems.MovingToStorage` names the offending furniture in the console when this trips.
...
- **Player-UI store/take is split between chest (server-authoritative) and bag/hands (per-peer).** The chest side is replicated by `_networkSlots` and mutated only on the server. The character side ([[character-equipment]] §"Replication contract") is **deliberately not** in any NetworkList — bag-inventory items and `HandsController.CarriedItem` live as independent per-peer copies. As a result the player-UI `RequestTakeServerRpc` / `RequestStoreFromBagServerRpc` / `RequestStoreFromHandsServerRpc` must respect the asymmetry: take routes via `CharacterActions.ReceiveItemPickupClientRpc` to the owner (server→client direction) and store includes the item payload + source slot index in the ServerRpc + uses `CharacterActions.RemoveFromInventoryAfterStoreClientRpc` to clear the owner's source (client→server direction). The chest side runs identically for host + remote clients (server-authoritative `NetworkList`); the asymmetry is only on the character side. NPC GOAP store/take paths (host-only) keep the original by-reference resolution via the 3-arg `CharacterStoreInFurnitureAction` constructor. See `CharacterStoreInFurnitureAction.OnApplyEffect` for the three-way source resolution (remote-client / host slot-index / legacy by-reference) and [[host-only-state-blindspot]] §"Bag-inventory state not replicated" for the audit case study.
...
- [x] **UI window.** Player-facing UI shipped 2026-05-09 — see [[storage-furniture-ui]].
...
- 2026-05-14 — `Role` is now also written by the new shift-punch storage-role assignment pass — see [[commercial-storage-roles]] and `BuildingLogisticsManager.AssignStorageRolesForShift`. No code change on `StorageFurniture` itself; the new pass routes through the existing `StorageFurnitureNetworkSync.SetRoleServer` mutator. — claude
- 2026-05-14 — Added runtime safety net in `Furniture.Awake` that auto-creates the `InteractionPoint` child when both authored Transform and `FurnitureInteractable.InteractionZone` are missing. Fixes the "LogisticsManager stalls in front of storage, then drops on the floor" symptom on any prefab/runtime-placed furniture that bypassed the editor `Reset()` hook. — claude
- 2026-05-14 — Aligned the arrival check in `GoapAction_GatherStorageItems.MovingToStorage` and `GoapAction_TakeFromSourceFurniture.MovingToFurniture` with the canonical Interactable Core Rule #1 pattern (`InteractableObject.IsCharacterInInteractionZone` + arrived-but-just-outside-2f softlock + legacy 1.5f fallback). The deposit/withdraw GOAP actions now match `GoapAction_FetchToolFromStorage` / `GoapAction_ReturnToolToStorage`. Worker no longer needs to reach the precise `_interactionPoint` — being anywhere inside the `FurnitureInteractable.InteractionZone` is sufficient, which is robust against `_interactionPoint` landing inside the `NavMeshObstacle` carve. — claude
- 2026-05-15 — Plugged the SellShelf routing leak: the generic first-fit walk in both `CommercialBuilding.FindStorageFurnitureForItem` and `GoapAction_GatherStorageItems.DetermineStoragePosition` now skips `Role == SellShelf` storages. Before this, a non-catalog item (e.g. a weapon in a bread shop) would fall through the catalog-gated shop-shelf pre-pass and first-fit-land on a SellShelf with any free slot, polluting the shop's sale display with items the shop doesn't sell. Symmetric to the existing tool-storage exclusion. — claude
...
- 2026-05-14 — Fixed the player-UI multiplayer store/take asymmetry on a joining client. `CharacterTakeFromFurnitureAction.OnApplyEffect` now routes the bag/hands delivery via `CharacterActions.ReceiveItemPickupClientRpc` for remote-client takers. `StorageFurnitureNetworkSync.RequestStoreFromBagServerRpc` / `RequestStoreFromHandsServerRpc` now accept `(FixedString64Bytes itemId, FixedString4096Bytes itemJson)` so the server can reconstruct the item without reading the client's (empty server-side) bag; the queued `CharacterStoreInFurnitureAction` gets a new 4-arg constructor + a 3-way `OnApplyEffect` that handles remote-client / host-slot-index / legacy by-reference sources. New `[Rpc(SendTo.Owner)] CharacterActions.RemoveFromInventoryAfterStoreClientRpc(int sourceSlotIndex)` clears the owner's local source after a successful chest insertion. NPC GOAP deposit paths (4 callsites in `GoapAction_DepositResources` / `GatherStorageItems` / `RestockSellShelves`) continue to use the 3-arg ctor with the legacy by-reference path — no GOAP changes. Late-joiner repro verified: client buys at vendor, takes from chest, stores from bag, stores from hands — all end up in the correct authoritative location on both peers, and chest contents persist through portal-gate/bed save. See [[host-only-state-blindspot]] §"Bag-inventory state not replicated" for the audit case study and `.agent/skills/item_system/SKILL.md` §"Bag-inventory replication authority" for the canonical pattern. — claude
