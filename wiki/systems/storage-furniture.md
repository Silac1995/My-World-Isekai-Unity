...
| File | Role |
|------|------|
| [StorageFurniture.cs](../../Assets/Scripts/World/Furniture/StorageFurniture.cs) | Slot-based container — data + API + lock state. Adds `ApplySyncedSlotsFromNetwork` + `ApplyLockStateFromNetwork` for the sync layer and `RestoreFromSaveData` for the save-restore path. |
...
- **Network sync (shipped 2026-04-25, lock replication shipped 2026-05-09)**: `StorageFurnitureNetworkSync` (sibling `NetworkBehaviour`) carries a server-write `NetworkList<NetworkStorageSlotEntry>` for slot contents plus a server-write `NetworkVariable<bool> _isLockedSync` for the lock flag. Each slot entry is `{ ushort SlotIndex, FixedString64Bytes ItemId, FixedString4096Bytes JsonData }`. Sparse — empty slots are absent. Server rewrites the list on every `OnInventoryChanged` (full clear + re-add of non-empty slots; O(Capacity), bounded ≤ 32 in the authored Crate); the same handler also pushes `_storage.IsLocked` into `_isLockedSync` whenever it differs (differ-check avoids spurious NetworkVariable writes). `Lock()` / `Unlock()` both fire `OnInventoryChanged`, making `HandleServerInventoryChanged` the single chokepoint for both slot and lock propagation. Clients listen on `OnListChanged` for slots (rebuilding via `StorageFurniture.ApplySyncedSlotsFromNetwork` on every event regardless of `EventType` — see [[multiplayer]] §8 NetworkList event-type fan-out gotcha), and on `_isLockedSync.OnValueChanged` for the lock flag (applying via `StorageFurniture.ApplyLockStateFromNetwork`). Late-joiners receive both through `ApplyFullStateOnClient` + `_isLockedSync.Value` reads in `OnNetworkSpawn`.
- **Persisted state (slots shipped 2026-04-25, lock shipped 2026-05-09)**: slot contents and lock state survive `MapController.Hibernate` / `WakeUp` and game-session reloads via `BuildingSaveData.StorageFurnitures` — a `List<StorageFurnitureSaveEntry>`, default-empty so older save files (no field) still deserialize cleanly. `StorageFurnitureSaveEntry` now carries `bool IsLocked` (defaults `false` so pre-2026-05-09 saves behave as unlocked — backward-compat, no migration needed). `BuildingSaveData.FromBuilding` snapshots both non-empty slots and `storage.IsLocked` into the entry. On load, `MapController.RestoreStorageFurnitureContents` rehydrates slots through `StorageFurniture.RestoreFromSaveData`, then calls `storage.ApplyLockStateFromNetwork(saved.IsLocked)` for the lock flag. `ApplyLockStateFromNetwork` early-returns when the value matches (`_isLocked` defaults false), so old saves with `IsLocked = false` pay zero cost. When it differs, it fires `OnInventoryChanged` which flows through `StorageFurnitureNetworkSync.HandleServerInventoryChanged` → `_isLockedSync.Value = true` — clients receive the lock state on connect with no extra restore-side networking code.
...
- [x] ~~**Lock state replication + persistence.**~~ Shipped 2026-05-09. `StorageFurnitureNetworkSync` now carries `NetworkVariable<bool> _isLockedSync` (server-write, everyone-read). `HandleServerInventoryChanged` pushes `_storage.IsLocked` whenever it differs — `Lock()` / `Unlock()` both fire `OnInventoryChanged`, so this handler is the single propagation chokepoint. Clients apply via `StorageFurniture.ApplyLockStateFromNetwork` in `_isLockedSync.OnValueChanged`. `StorageFurnitureSaveEntry` carries `bool IsLocked` (default `false` for backward-compat). `BuildingSaveData.FromBuilding` captures it; `MapController.RestoreStorageFurnitureContents` restores it via `ApplyLockStateFromNetwork`. See "State & persistence".
...
- 2026-04-25 — Shipped `StorageFurnitureNetworkSync` (sibling NetworkBehaviour authored on `Storage.prefab`, inherited by every variant). Server now replicates slot contents via `NetworkList<NetworkStorageSlotEntry>`; clients mirror through new `StorageFurniture.ApplySyncedSlotsFromNetwork` and fire `OnInventoryChanged` locally so `StorageVisualDisplay` rebuilds on every peer. — claude
...
- 2026-04-25 — Shipped save/restore for slot contents. New types `StorageSlotSaveEntry`, `StorageFurnitureSaveEntry`, and `BuildingSaveData.StorageFurnitures` (default-empty for backward-compat) in [MapRegistry.cs](../../Assets/Scripts/World/MapSystem/MapRegistry.cs). New static helper `BuildingSaveData.ComputeStorageFurnitureKey` for the composite (FurnitureItemSO.ItemId @ building-local-position) furniture key. `StorageFurniture.RestoreFromSaveData` is the new server-only restore entry point. `MapController.RestoreStorageFurnitureContents` is invoked from both `SpawnSavedBuildings` and `WakeUp` after the building's default-furniture spawn finishes. The same `OnInventoryChanged` flow used at runtime drives the network-sync rewrite, so late-joining clients see restored state on connect — no extra networking code needed. — claude
- 2026-05-09 — Shipped lock-state replication + persistence. Added `NetworkVariable<bool> _isLockedSync` (server-write, everyone-read) to `StorageFurnitureNetworkSync`; extended `HandleServerInventoryChanged` to push `_storage.IsLocked` when it differs; added `HandleLockChanged` to mirror into `StorageFurniture.ApplyLockStateFromNetwork` on all peers. New method `StorageFurniture.ApplyLockStateFromNetwork(bool)` sets `_isLocked` directly and fires `OnInventoryChanged` once (bypasses Lock/Unlock to avoid feedback loop). Added `bool IsLocked` to `StorageFurnitureSaveEntry` (default `false`, backward-compat); `BuildingSaveData.FromBuilding` captures it; `MapController.RestoreStorageFurnitureContents` restores it via `ApplyLockStateFromNetwork` after slot restore. Old saves load as `IsLocked = false` — no migration. — claude
