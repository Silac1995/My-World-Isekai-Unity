...
- 2026-04-30 — Implemented `DialogueManager`-style advance behaviour: `SpeakStep` and `DialogueStep` now check `ctx.ParticipatingPlayers.Count > 0` and either wait for a player press (Space / Left-Click forwarded by `PlayerController` via the new `CinematicAdvance` static helper) or auto-advance after the existing 1.5s dwell. Closes the gap with the legacy `DialogueManager._hasPlayer` branching. Phase 2 will replace the single-frame static flag with a server-side AllMustPress press tally + ServerRpc per client. — Claude / [[kevin]]
