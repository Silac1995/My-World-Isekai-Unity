...
| [Steps/](../../Assets/Scripts/Cinematics/Steps/) | `WaitStep`, `SpeakStep`, `MoveActorStep`, `TriggerStep`, `DialogueScriptStep` (Phase 1 set; the script step wraps a legacy `DialogueSO` so multi-line conversations don't have to be re-authored as individual SpeakSteps). |
...
- 2026-04-30 — Added `DialogueScriptStep` so designers can wrap an existing legacy `DialogueSO` as a single step in the cinematic timeline. Maps `DialogueLine.CharacterIndex` (1-based) to roles via a designer-authored list; supports both legacy `[indexN].getName` and new `[role:X].getName` placeholder syntax. Reuse helper — no need to re-author multi-line conversations as individual `SpeakStep`s. Per-line auto-advance (1.5s post-typing dwell) matches `SpeakStep`'s Phase 1 cadence. — Claude / [[kevin]]
