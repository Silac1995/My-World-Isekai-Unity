...
- 2026-05-17h — **Reputation penalty rebalanced: −5 → −2 (supplier + transporter).** User-locked deltas: +1 on full BuyOrder completion (unchanged), −2 on order expiration for both the supplier (`expired.Source` if it's a CommercialBuilding) and every uncompleted transporter (`TransportOrder.HostTransporter`). Symmetric — both parties share the same penalty when the chain fails. Touches `BuildingLogisticsManager.CheckExpiredBuyOrders` only (two literals). — claude
