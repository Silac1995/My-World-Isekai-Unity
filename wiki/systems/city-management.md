...
- **Join-request lifecycle**: `JoinRequestDesk : Furniture` (spontaneous one-shot — actor taps E, request submits, BT keeps going; previously `OccupiableFurniture` with a sit-and-wait seat, reverted 2026-05-18) forwards drifter interactions to `AB.SubmitJoinRequestServerRpc`; AB maintains a replicated `NetworkList<JoinRequest>`. Leaders process via `AcceptJoinRequestServerRpc` / `DeclineJoinRequestServerRpc`. Accept fires `AddMember` + `JoinCommunity` + `SetCitizenship` + `ClearCurrentAction`. The interacting player gets a local toast via `_toastChannel` (Cashier / CharacterEquipment pattern: `IsPlayer()` + `IsOwner` gates) — the server RPC stays authoritative; the toast is pure UX confirmation that E was registered.
...
- 2026-05-18 — **JoinRequestDesk simplified from `OccupiableFurniture` to `Furniture` — spontaneous one-shot, no sit-and-wait.** The seat metaphor added nothing for a fire-and-forget submission and blocked the actor's BT from continuing onto the next ambition step. The desk now raises a local toast on the interacting player's HUD (`IsPlayer()` + `IsOwner` gates per rule #19b) via a new `_toastChannel` SerializeField. Server-side `AB.SubmitJoinRequestServerRpc` stays authoritative. — claude
