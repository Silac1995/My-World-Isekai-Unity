...
- 2026-05-18 — Plan 4b: InitializeJobs now staffs the AB with JobBuilder × 2 + JobHarvester + JobLogisticsManager. New unfulfillable-material harvest queue (`_unfulfillableMaterialHarvestQueue`, `EnqueueUnfulfillableMaterial`, `GetUnfulfillableHarvestQueue`, `DecrementUnfulfillableMaterial`) backs the JobLogisticsManager → JobHarvester handoff for materials the logistics chain couldn't source. JobLogisticsManager.ProcessActiveBuildOrders cascade ships (B2B → producer → virtual → unfulfillable-queue) along with four NEW JobBuilder GoapActions and the JobBuilder class itself. JobHarvester's CityHarvester runtime branch ships the full 7-state state machine (FindTarget → MoveToTarget → Harvesting → PickupDroppedItem → MoveToABStorage → DepositItem) that drains the AB queue via the standard CharacterHarvestAction + CharacterPickUpItem + CharacterStoreInFurnitureAction primitives. Pipeline ships fully functional end-to-end. — claude
- 2026-05-18 — **Plan 4c skeleton + UI scripts shipped.** Tier-up: `Community.TryPromoteLevel(ab)` (POCO method gating on Population/Treasury/RequiredBuildings against `CommunityTierRequirementsSO`) + `AB.RequestPromoteLevelServerRpc` (leader-authority gate + `TierUpResultClientRpc` reply). Civic placement: `AB.PlaceCityBlueprintServerRpc(prefabId, targetCell)` validates leader-authority + Civic-category + tier-unlock + `BuildingGrid.CanPlace`, delegates to `BuildingPlacementManager.PlaceCivicBuildingForLeader` (NEW server-only spawn helper), wires `community.ownedBuildings` + multi-owner + `BuildingGrid.Register`, auto-creates a `BuildOrder` on this AB. Join-request flow: `NetworkList<JoinRequest>` `PendingJoinRequests` + `SubmitJoinRequestServerRpc` + `AcceptJoinRequestServerRpc` + `DeclineJoinRequestServerRpc` (with `JoinRequestDesk : Furniture` — spontaneous one-shot since the 2026-05-18 design slip, was briefly `OccupiableFurniture` — as the in-world interaction surface). Daily migration: `DrifterMigrationSystem : MonoBehaviour` on `MapController` subscribes to `TimeManager.OnNewDay`, spawns up to N drifters at random NavMesh-sampled map edges per chartered community (designer-wired `_drifterPrefab` — unset = log + skip). Player-facing UI: `CityManagementFurniture : Furniture` (leader-only interactable) + `UI_CityManagementPanel : UI_WindowBase` variant with three tabs (TierUp / PlaceBuilding / JoinRequests). PlayerUI gains `OpenCityManagementWindow / CloseCityManagementWindow / IsCityManagementWindowOpen`. 9 commits, 214 EditMode tests pass. **Deferred follow-ups**: AB.prefab preplaced furniture wiring, UI_CityManagementPanel.prefab variant authoring, PlayerController CityPlacementGhost cursor-mode hand-off. — claude
