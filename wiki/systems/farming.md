...
**Layer 5 — UI input**
- `PlayerController` E-key dispatcher (rule #33): Seed/WateringCan held → placement; placement active → no-op; consumable held → consume; otherwise tap-E → `Interact()` on nearest visible interactable, hold-E (≥0.4s) → open the unified `UI_InteractionMenu` via `PlayerUI.OpenInteractionMenu` with rows from `Harvestable.GetHoldInteractionOptions(actor)`.
- `UI_InteractionMenu` (button-bar surface used by doors, beds, harvestables, …) consumes `List<InteractionOption>`. `InteractionOption` carries the optional rich affordances `Icon`, `Subtext`, `DisabledReason`; the menu binds them when the spawned button prefab exposes child components named `Icon` (Image), `Subtext` (TMP_Text), `Reason` (TMP_Text), and degrades to text-only when those slots are absent — preserving the existing simple-button behaviour for legacy callers.
...
**UI:** [Assets/Scripts/UI/WorldUI/UI_InteractionMenu.cs](../../Assets/Scripts/UI/WorldUI/UI_InteractionMenu.cs) — the project-wide button-bar surface. The hold-E menu used to live in a parallel `UI_HarvestInteractionMenu` + `HarvestInteractionOption` + `UI_HarvestInteractionOptionRow` triple under `Assets/Scripts/UI/Interaction/`; that pipeline was removed 2026-05-12 and `InteractionOption` itself gained the optional `Icon`/`Subtext`/`DisabledReason` fields the harvest menu needs (graceful prefab-driven binding).
...
- `Assets/Scripts/Interactable/InteractionOption.cs` — extended with optional `Icon` / `Subtext` / `DisabledReason` fields (2026-05-12 unification).
