...
## Change log
- 2026-05-14 — **Dirty-flag gating for `GoapAction_RestockSellShelves`.** Mirrors the `LogisticsOrderBook._dispatchDirty` pattern. New server-only field `_restockDirty` on `BuildingLogisticsManager` + public API `IsRestockDirty()` / `MarkRestockDirty()` / `ClearRestockDirty()`. Sources: (a) `CommercialBuilding.AddToInventory` / `TakeFromInventory` / `RemoveExactItemFromInventory` (siblings of the existing `MarkDispatchDirty` calls), (b) per-storage `StorageFurniture.OnInventoryChanged` (subscriptions managed in `BuildingLogisticsManager.RefreshStorageContentSubscriptions`, refreshed from the existing `CommercialBuilding.GetStorageFurnitureCached` chokepoint right after `RefreshStorageRoleSubscriptions`), (c) building-level `OnStorageRolesChanged` and `ShopBuilding.OnCatalogChanged` (subscribed once in `Awake`, unbound in `OnDestroy`), (d) tail of `AssignStorageRolesForShift` when any role was written, (e) tail of `GoapAction_RestockSellShelves.Execute` after a successful slot transfer. `IsValid` gate: `if (logistics != null && !logistics.IsRestockDirty()) return false;` before the expensive `FindMisplacedSaleItem` walk; cleared inside `IsValid` only when the scan proves zero candidates. The carried-item early-return runs before the gate (must complete in-flight transfers regardless). Initial state = dirty (warm-start, load-from-save). Server-only state, no replication. See [[performance-conventions]] Pattern 1 for the canonical shape. — claude
- 2026-05-14 — **Sell-shelf restock loop for shops.** Two-part fix: (a) **routing-side** — `CommercialBuilding.FindStorageFurnitureForItem` adds a SellShelf pre-pass (after the existing tool pre-pass) when the building is a `ShopBuilding` AND the item is in its `GetCatalogEntry` catalog, so supplier deliveries of sale goods land on a shelf directly. (b) **heal-existing-state** — new `GoapAction_RestockSellShelves` (cost 0.3f) added to `JobLogisticsManager._availableActions`. Walks every non-SellShelf `StorageFurniture` in the shop, finds catalog-matching items, transports them slot→slot to a SellShelf with free capacity. Reuses existing `CharacterTakeFromFurnitureAction` + `CharacterStoreInFurnitureAction` (no new actions); ignores items currently reserved by outbound `TransportOrder`s (defers to `GoapAction_StageItemForPickup`). Server-authoritative (slot mutations land server-side via the existing actions); idempotent (planner re-fires until shelves are full or sources empty); zero new `TransportOrder` schema — same-building intra-storage moves are pure CharacterAction. `GoapAction_GatherStorageItems.DetermineStoragePosition` mirrored the same SellShelf pre-pass so loose item gather inside a shop also routes to the shelf first. Cost ordering: PlaceOrder 0.1 < StageForPickup 0.2 < **RestockSellShelves 0.3** < Gather 0.5 < Idle. — claude
- 2026-05-14 — `AssignStorageRolesForShift` now routes through `CommercialBuilding.TrySetStorageRoleServer` (the canonical helper) instead of writing `StorageFurnitureNetworkSync.SetRoleServer` directly. Player UI's RPC body and this NPC pass therefore share the same `DoSetStorageRole` invocation, so any side-effect added to one path is automatically picked up by the other. The local `IsRoleSupported` fast-path bail is kept at the call site (avoids per-storage previousRole-capture + log-string formatting when the building's subtype catalog already says no); authoritative filtering lives inside the helper. Playtest-confirmed. See [[commercial-storage-roles]]. — claude
...
## Sources
- [[performance-conventions]] — Pattern 1 (dirty-flag gating) was extracted from this system's `_dispatchDirty` implementation. Read before adding any new dispatcher state.
- [[optimisation-backlog]] — Tier 3 B measurements + Tier 4 deferrals.
