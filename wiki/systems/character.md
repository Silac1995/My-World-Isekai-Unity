...
- `character.OccupyingFurniture` (read-only property) — server-replicated since 2026-05-14 via `NetworkOccupyingFurnitureNetId` (`NetworkVariable<ulong>`, EveryoneRead / ServerWrite). Server-write happens inside `SetOccupyingFurniture`; the getter on clients resolves the NetworkObject via `NetworkManager.SpawnManager` (`GetComponent<Furniture>` fast path for self-attached NOs like Cashier, `GetComponentInChildren<Furniture>` fallback for Bed/Chair under a building NO). `OnValueChanged` fires `OnOccupyingFurnitureChanged` on remote peers so any `CharacterSystem.HandleOccupyingFurnitureChanged` listener stays accurate cross-peer. Required so the literal "OccupyingFurniture != null ⇒ no movement" gate fires correctly on every peer (rule #19b).
...
## Change log
- 2026-05-14 — Replicated `Character.OccupyingFurniture` via the new `NetworkOccupyingFurnitureNetId` (`NetworkVariable<ulong>`). Server-write only; client getter resolves through `NetworkManager.SpawnManager`. `OnValueChanged` fires `OnOccupyingFurnitureChanged` on remote peers. Required precondition for the `CharacterAction_OccupyFurniture` refactor that replaced `Cashier.ServerTickAutoOccupy` — the spec's literal "OccupyingFurniture != null ⇒ no movement" gate needs to fire on every peer (rule #19b). New continuous action `CharacterAction_OccupyFurniture` is server-only, lifecycle `OnStart` → `Use`, `OnTick` → validate, `OnCancel` → `Leave` (idempotent). Replaces the proximity tick with the canonical action pipeline; same action queued from player E-press paths (CashierInteractable / OccupiableFurniture.OnInteract → CharacterActions.RequestOccupyFurnitureServerRpc / CashierNetSync.RequestUseCashierServerRpc) and NPC `JobVendor.Execute` step 3 — controller swaps no-op for seating state (rule #22 player↔NPC parity). Movement locked while seated in `PlayerController.Move`, `CharacterMovement.SetDestination`, `CharacterMovement.SetDesiredDirection`. See [docs/superpowers/specs/2026-05-14-furniture-occupancy-via-characteraction-design.md](../../docs/superpowers/specs/2026-05-14-furniture-occupancy-via-characteraction-design.md). — claude
