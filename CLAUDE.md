...
39. **Any UI window / surface that can be closed MUST be a Prefab Variant of [Assets/UI/Player HUD/UI_WindowBase.prefab](Assets/UI/Player%20HUD/UI_WindowBase.prefab) and live under `Assets/UI/Player HUD/`.** The defining trait is **"has a close affordance"** — not size or modality. Full-screen panels, half-screen modals, side drawers, mid-screen popups, confirmation dialogs, tooltips that the user can dismiss, sub-windows opened from inside another window — all of them MUST inherit `UI_WindowBase`. The only UI prefabs that are *not* subject to this rule are leaf elements that have no close button and live as `Instantiate`d children inside their parent window (rows, tiles, list items, badges).
    ...
    **Concretely:**
    ...
    - **NEVER put a `ContentSizeFitter` on `Panel_Main_Background` or any other fixed-size frame in a variant.** Unity's layout-rebuild pass fires AFTER `LateUpdate` and BEFORE rendering — every script-side write to the frame's `sizeDelta` (from `Awake`/`OnEnable`/`Initialize`/`LateUpdate`) gets clobbered within the same frame. The base `UI_WindowBase.prefab` shipped one historically (removed 2026-05-16). `ContentSizeFitter` IS correct on `ScrollView/Viewport/Content` (vertical-fit only — needed for scrolling). Anywhere else on a window = bug. Diagnostic: `panelRoot.GetComponentsInChildren<ContentSizeFitter>(true)` lists all owners.
...
## Furniture & Building Prefab Hierarchy

40. **Every furniture prefab MUST be a Prefab Variant of [Assets/Prefabs/Furniture/Furniture_prefab.prefab](Assets/Prefabs/Furniture/Furniture_prefab.prefab); every building prefab MUST be a Prefab Variant of [Assets/Prefabs/Building/Building_prefab.prefab](Assets/Prefabs/Building/Building_prefab.prefab).** The root base prefabs supply the shared scaffold (zone collider, network identity policy, component baseline) that every furniture or building inherits — no concrete prefab re-implements that scaffold from scratch. This mirrors rule #39 (UI HUD Prefab Architecture) for the gameplay-prefab side.

    The hierarchy is **two-tier**:
    - **Tier 1 — Type base**: a direct Prefab Variant of the root base, one per furniture/building *type*. The type base owns the scripts and components shared by every variant of that type. Examples under `Assets/Prefabs/Furniture/`: `Bed.prefab`, `Cashier.prefab`, `CraftingStation.prefab`, `TimeClock.prefab`, `Storage/Crate.prefab`, `Storage/Storage.prefab`, `Safe/Safe Base.prefab`, `Management/Commercial Console.prefab`. Examples under `Assets/Prefabs/Building/`: `Commercial/CommercialBuilding_prefab.prefab` (carries `CommercialBuilding` + the shared `BuildingTaskManager` / `BuildingLogisticsManager`), `House/House prefab.prefab`.
    - **Tier 2 — Specific variant**: a Prefab Variant of the matching **type base**, one per concrete piece of authored content. Examples: `Safe/Safe.prefab` (variant of `Safe Base`), `Storage/Storage Visible Items.prefab` (variant of `Storage`), `Commercial/Shop/Shop.prefab` / `Commercial/Shop/Clothing Shop.prefab` / `Commercial/Farm/Farming Building.prefab` / `Commercial/Crafting/Forge.prefab` / `Commercial/Harvesting/Lumberyard/Lumberyard.prefab` / `Commercial/Transporter/Transporter Building.prefab` / `Commercial/Administrative/AdministrativeBuilding.prefab` (all variants of `CommercialBuilding_prefab`), `House/Small house/Small house.prefab` / `House/Medium house/Medium house.prefab` (variants of `House prefab`).

    **Concretely:**
    - **Asset location**: furniture prefabs MUST live under `Assets/Prefabs/Furniture/`; building prefabs under `Assets/Prefabs/Building/`. The folder layout mirrors the hierarchy — the root base sits at the folder root; type bases live either at the folder root (single-line types like `Bed`, `Cashier`) or inside a type subfolder (`Storage/`, `Safe/`, `Commercial/`, `House/`, `Management/`); specific variants live inside the matching type subfolder. Never put a gameplay-prefab in `Assets/Prefabs/` root, in `Assets/UI/...`, or anywhere outside the two roots above.
    - **Authoring a new TYPE** (rare — a brand-new kind of furniture or building) = create a Prefab Variant of the **root base** (`Furniture_prefab.prefab` / `Building_prefab.prefab`), place it in the matching folder, attach the type-specific script (a new `Furniture` subclass, or a `Building` / `CommercialBuilding` subclass), then save. Use `PrefabUtility.InstantiatePrefab(rootBase)` + `PrefabUtility.SaveAsPrefabAsset` from a Roslyn snippet, or right-click → Create → Prefab Variant in the Project view. Verify the inheritance with `PrefabUtility.GetCorrespondingObjectFromSource(saved).name == "Furniture_prefab"` (or `"Building_prefab"`).
    - **Authoring a new specific variant** (common — a new crate, a new shop, a new house) = create a Prefab Variant of the matching **type base**, NEVER of the root base. A new crate variant goes from `Storage/Crate.prefab`, not from `Furniture_prefab.prefab`. A new shop goes from `Commercial/Shop/Shop.prefab` (or `CommercialBuilding_prefab` if it adds shop-distinct behavior). This way the type base remains the single place to tweak shared behavior across every variant of that type — overriding a script reference on a Tier-2 variant defeats the inheritance.
    - **Backing script** must inherit the matching base (`Furniture` subclass for furniture; `Building` / `CommercialBuilding` / `ShopBuilding` / `FarmingBuilding` / `AdministrativeBuilding` / etc. for buildings). The script reference lives on the **type-base** prefab (or higher); specific Tier-2 variants override SerializeFields (icon, name, references to a `BuildingSO`/`FurnitureSO`), NOT script identity.
    - **Network identity rule**: furniture baked into a runtime-spawned building prefab MUST NOT have its own `NetworkObject` (see [[no-nested-networkobject-in-runtime-spawned-prefab]] gotcha). The `Furniture_prefab` root base MUST NOT carry a `NetworkObject`. Any networked behavior on a furniture instance is replicated via the parent building's `NetworkObject`. Buildings DO carry their own `NetworkObject` on the `Building_prefab` root base; every building variant inherits it.
    - **Default-furniture layout schema**: `BuildingSO._defaultFurnitureLayout` references furniture prefabs by asset reference + slot position. Slot positions feed `FurnitureKey` (the storage save key). Repositioning a slot after a save silently drops the storage contents — treat slot positions as **immutable post-ship** (see `feedback_layout_position_save_schema`).
    - **Type-base script & component edits propagate** to every Tier-2 variant via Unity's prefab-variant override system. When you edit the type base, every concrete prefab inherits the change automatically — that's the whole point. Avoid duplicating components on Tier-2 variants; if you find yourself adding the same component to three crate variants, lift it to `Crate.prefab`.

    **Matching ScriptableObject is mandatory (the prefab alone is NOT enough).** Every shipped furniture prefab AND every shipped building prefab MUST have a matching ScriptableObject asset authored at the same time as the prefab. The prefab is the visual/component container; the SO is the data/registry record (PrefabId, display name, icon, recipe, footprint, treasury seed). Both are needed — a prefab without its SO has no name, no icon, no construction recipe, no save-key identity, no registry entry, and will not survive save/load or appear in the placement UI.

    - **Furniture SO** = `FurnitureItemSO` (subclass of `ItemSO`) at `Assets/Resources/Data/Item/Furniture/<Name>.asset`. Create via `Create → Scriptable Objects → Items → Furniture` in the Project view. Holds the **bidirectional link** with the furniture prefab: `FurnitureItemSO._installedFurniturePrefab` ↔ `Furniture._furnitureItemSO` SerializeField on the prefab. **Both fields must be wired** — wiring only one direction silently breaks pickup (you can place the furniture but not pick it back up, or vice versa). Existing examples: `Cashier.asset`, `CommercialConsole.asset`, `CraftingStation.asset`, `Crate.asset`, `Safe.asset`, `Time Clock.asset`. `CharacterPickUpFurnitureAction` routes through this SO to convert the world Furniture back to an ItemInstance.
    - **Building SO** = `BuildingSO` (or `BuildingCommercialSO` subclass for buildings with treasury / jobs / logistics — Shop, Forge, Farming, Transporter, Harvesting, Administrative) at `Assets/Resources/Data/Buildings/<Name>.asset`. Create via `Create → MWI → World → BuildingSO` / `BuildingCommercialSO` in the Project view. Holds `_prefabId` (stable cross-session save-key — see immutability rule below), `BuildingName`, `Icon`, `BuildingPrefab` reference, `InteriorPrefab` reference, `BuildingType`, `ConstructionRequirements`, `DefaultFurnitureLayout`, `GridFootprintCells`, `BlueprintCategory` (`Personal`/`Civic`), `MinTier`. `BuildingCommercialSO` adds `BaseTreasury`. Existing examples: `B000-Lumberyard.asset`, `B001-Crafting.asset`, `B002-SmallHouseA.asset`, `B003-MediumHouseA.asset`, `B004-ClothingShop.asset`, `B005-TransportBuilding.asset`, `B007-FarmBuilding.asset`, `AdministrativeBuilding.asset`. The prefab's `Building._blueprint` SerializeField points back at this SO; every accessor (`BuildingType`, `BuildingName`, `ConstructionRequirements`, `DefaultFurnitureLayout`, `GridFootprintCells`) reads through `_blueprint`. Without `_blueprint` wired, the building has no identity at runtime.
    - **Register the BuildingSO in `WorldSettingsData.BuildingRegistry`** (the `List<BuildingSO>` field on the WorldSettingsData asset; formerly named `Blueprints`). Building lookup, hibernation/wake-up restore, placement UI, and community-tier unlock paths all scan this registry by `PrefabId`. **A BuildingSO that exists on disk but is NOT in `BuildingRegistry` is dead weight — nothing will find it.** No equivalent global registry for `FurnitureItemSO`; furniture is discovered via the `BuildingSO._defaultFurnitureLayout` references + `Resources.LoadAll` lookups on the item layer.
    - **PrefabId immutability**: `BuildingSO._prefabId` is the cross-session save-key. Renaming it silently invalidates every existing save that references that building type — every saved instance becomes orphaned at load time. **Treat as immutable post-ship.** Same constraint applies to slot positions inside `_defaultFurnitureLayout` (see `feedback_layout_position_save_schema`).
    - **Network-prefab registration**: every building prefab carries a `NetworkObject` (inherited from `Building_prefab.prefab`), so every Tier-2 building variant must be added to NGO's `DefaultNetworkPrefabsList` so clients can spawn it. The Tier-1 type-base inheritance does NOT cover this — each concrete variant is its own network-prefab entry. `Furniture_prefab.prefab` does NOT carry a `NetworkObject`, so most furniture variants do not need a network-prefab entry; the exceptions are furniture that ships as a runtime-spawned standalone NetworkObject (rare — most are baked into a building's `_defaultFurnitureLayout` and converted via `Building.ConvertNestedNetworkFurnitureToLayout`).

    **Building-chain scripts MUST live on Tier-2 — never lift them to root or Tier-1 (anti-pattern, 2026-05-19 editor-crash incident).** The `Building → ComplexRoom → Room → Zone` chain is polymorphic via `NetworkBehaviour`, and these `[RequireComponent]` declarations form a transitive trap: `Zone` requires `BoxCollider` + `NavMeshModifierVolume`; `Room` requires `FurnitureGrid` + `FurnitureManager`; `BuildingInteractable` and `ConstructionSiteScanner` each require `Building`; `CommercialBuilding` requires `BuildingTaskManager` + `BuildingLogisticsManager`. The concrete subclass identity (`ShopBuilding`, `ForgeBuilding`, `FarmingBuilding`, `HarvestingBuilding`, `TransporterBuilding`, `AdministrativeBuilding`) lives on the **Tier-2 specific variant** — it has to, because each type needs its own subclass.

    If you add `Building`, `FurnitureGrid`, `FurnitureManager`, `BuildingInteractable`, `ConstructionSiteScanner`, `BuildingTaskManager`, `BuildingLogisticsManager`, `BoxCollider`, or `NavMeshModifierVolume` to the **root** `Building_prefab.prefab` OR to a **Tier-1 type base** (`CommercialBuilding_prefab.prefab`, `House prefab.prefab`), Unity auto-adds a base `Building` to satisfy the `[RequireComponent]`. Every Tier-2 variant then carries **two `Building`-chain `NetworkBehaviour`s** — the inherited base + its own `ShopBuilding`/`CommercialBuilding`/etc. subclass. At spawn time `Building.Awake → base.Awake() → ConvertNestedNetworkFurnitureToLayout` runs **twice**; the second pass `DestroyImmediate`s already-destroyed nested NetworkObject children → null deref inside Unity's native scene-graph → **whole Editor crashes** (no managed exception; `Assets/_Recovery/*.unity` files pile up — a cluster of 5+ within 20 minutes is the canonical signature).

    These scripts belong on the **Tier-2 specific variant**, and only there. The duplication across variants is *content* duplication (each variant has its own configured instance bound to that variant's subclass) — it is NOT the kind of duplication that lifting can eliminate, because polymorphism + `[RequireComponent]` + NGO's positional `NetworkBehaviour`-index assignment forces this shape. Tier-1 type bases (`CommercialBuilding_prefab.prefab`, `House prefab.prefab`) carry `m_AddedComponents: []` at HEAD — they exist purely as an inheritance anchor that gives Tier-2 variants a common folder + a hook for future genuinely-shared content. The root `Building_prefab.prefab` carries only `NetworkObject` + `BuildingConstructionOutline`. **Litmus test before adding any component to root or Tier-1**: grep the script for `[RequireComponent]`; if the chain transitively requires `Building`, or if the script itself extends `Zone`/`Room`/`ComplexRoom`/`Building`, do NOT add it to a parent prefab. The lift is structurally impossible without first removing the `[RequireComponent(typeof(Building))]` attributes from `Assets/Scripts/World/Buildings/Construction/BuildingInteractable.cs:20` and `Assets/Scripts/World/Buildings/Construction/ConstructionSiteScanner.cs:16` (and refactoring those scripts to resolve `Building` via runtime `GetComponentInParent`). Even then, the lift is a coordinated multi-prefab change that shifts NetworkBehaviour indices and breaks existing saves — **not worth the effort; the per-variant pattern is the working shape.**

    **Recovery (if you already crashed the editor):** (1) `git diff --stat -- Assets/Prefabs/Building/` to spot which prefabs were touched + count `Assets/_Recovery/*.unity` to confirm; (2) `git diff -- <suspect prefab>` and look for `+  m_EditorClassIdentifier: Assembly-CSharp::(Building|FurnitureGrid|FurnitureManager|BuildingInteractable|ConstructionSiteScanner)` on the root or Tier-1; (3) `git diff HEAD -- <those prefabs> > .claude/snapshots/<date>-building-root-restructure.patch` so the user's work isn't lost; (4) `git checkout HEAD -- Assets/Prefabs/Building/Building_prefab.prefab "Assets/Prefabs/Building/House/House prefab.prefab" Assets/Prefabs/Building/Commercial/CommercialBuilding_prefab.prefab` to revert the prefabs that cascade duplicates to every variant; (5) for any Tier-2 commercial variant the user stripped scripts from, Roslyn-re-add them on the variant root via idempotent `PrefabUtility.LoadPrefabContents → AddComponent if GetComponent==null → SaveAsPrefabAsset → UnloadPrefabContents` (the script set is per-variant — diff against HEAD before deciding); (6) `Assets → Refresh` (Ctrl+R) so AssetDatabase picks up the Roslyn writes; (7) verify per-script counts via `grep -c "::<sym>$" <prefab>` match HEAD. Full diagnostic + recovery walkthrough lives at [wiki/gotchas/duplicate-building-component-on-prefab-variant-root.md](wiki/gotchas/duplicate-building-component-on-prefab-variant-root.md).

    - The `building-furniture-specialist` agent in `.claude/agents/` is the deep-dive specialist for this hierarchy. Spawn it whenever you create, modify, debug, or audit any furniture/building prefab.

    See [wiki/systems/building.md](wiki/systems/building.md) and [wiki/systems/furniture-grid.md](wiki/systems/furniture-grid.md) for the architecture pages, [.agent/skills/building_system/SKILL.md](.agent/skills/building_system/SKILL.md) for the procedural skill, and the `building-furniture-specialist` agent for the deep-dive specialist.
