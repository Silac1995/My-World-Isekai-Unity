...
39. **Every player-facing HUD panel / modal window / full-screen UI surface MUST be a Prefab Variant of [Assets/UI/Player HUD/UI_WindowBase.prefab](Assets/UI/Player%20HUD/UI_WindowBase.prefab) and live under `Assets/UI/Player HUD/`.** The base prefab supplies the standard window chrome (Canvas + GraphicRaycaster + CanvasScaler + `Panel_Main_Background` Image + the inherited `_buttonClose` Button wiring). Variants inherit that chrome and add their own content on top — no panel re-implements the canvas/raycaster/background trio.

    **Concretely:**
    - **Asset location**: new variant goes in `Assets/UI/Player HUD/` (alongside `UI_StorageFurniturePanel.prefab`, `UI_SafePanel.prefab`, etc.). Never put panel variants in `Assets/Prefabs/` or any other UI folder.
    - **Prefab Variant relationship**: when you author the new panel, base it on `UI_WindowBase.prefab` (right-click in Project view → Create → Prefab Variant, or via `PrefabUtility.InstantiatePrefab(baseWindowAsset)` + `PrefabUtility.SaveAsPrefabAsset`). Verify after authoring: `PrefabUtility.GetCorrespondingObjectFromSource(savedPanel).name == "UI_WindowBase"`.
    - **Backing script must inherit [UI_WindowBase](Assets/Scripts/UI/UI_WindowBase.cs)**. The base auto-wires the inherited `_buttonClose` in `Awake` (so the close button "just works" without per-panel listener setup) and exposes `OpenWindow` / `CloseWindow` (the canonical activation pair). Override `CloseWindow` to add cleanup (unsubscribe events, clear rows, etc.) and call `base.CloseWindow()` last.
    ...
    - **Singleton entry-point lives on [PlayerUI.cs](Assets/Scripts/UI/PlayerUI.cs)**. Expose `Open<Name>Panel(... contextual args ...)` and `Close<Name>Panel()` as the only public surface — every caller (`Furniture.OnInteract`, `CharacterAction` ClientRpcs, hold-E menu options) routes through these. The panel itself is a `[SerializeField] private UI_<Name>Panel _xxxPanel;` field on `PlayerUI`, assigned in the scene under the `UI_PlayerHUD` GameObject (a scene-resident child of PlayerUI, never instantiated at runtime).
    - **Open<Name>Panel must `Debug.LogWarning` when `_xxxPanel` is null** so the diagnostic surfaces in the Console when a designer forgets to wire the SerializeField. Pattern: `if (_xxxPanel == null) { Debug.LogWarning("<color=orange>[PlayerUI]</color> Open<X>Panel called but _<x>Panel SerializeField is null — author the prefab and wire it to PlayerUI._<x>Panel in the Inspector."); return; }`. Saves hours of "why doesn't anything happen?" debugging.
    - **Non-panel UI elements (rows, tiles, list items, popups inside a panel) are NOT subject to this rule** — they are leaf UI prefabs (e.g. `UI_SafeCurrencyRow.prefab`, `UI_ShopBuyRow.prefab`) and are stored alongside their parent panel. The parent panel holds a reference to the row prefab via its own `[SerializeField] private UI_<Name>Row _rowPrefab` field and `Instantiate`s rows at runtime under a `_rowContainer` RectTransform.
