...
| `Assets/Scripts/Character/CharacterActions/CharacterAction_UseItem.cs` | **new** | Server-authoritative. Dispatches to the item's "use" behavior (eat food, drink potion). v1 supports `FoodInstance`-style consumables. **Blocker for this file:** the "consumable detection" question in §10 must resolve before this class can be authored — the dispatch surface (`IUsable` marker vs `ItemSO.IsConsumable` flag vs runtime type check) is the action's core contract, not a §10 nice-to-have. |
| `Assets/Scripts/Character/CharacterActions/EquipmentSourceRef.cs` | **new** | Tiny serializable discriminated value (`{ Kind, BagIndex?, Layer?, Slot? }`) shared by `CarryInHand` / `StashInBag` / `UseItem` to identify the source uniformly. See §8 for shape. |
...
- **Equip-side displacement behavior change.** Sweeping all current callers of `CharacterEquipment.Equip` to confirm none depend on the "displaced wearable drops to ground" side-effect. Known callsites to verify at plan-phase: NPC GOAP equip paths (clothing job, armor pickup), `WorldItem.RequestInteractServerRpc` pickup, `CharacterEquipment.Deserialize` save-load, any future ambition / cinematic step that auto-equips. Plan-phase should `grep "CharacterEquipment.Equip("` + `grep ".Equip("` in the equipment owner_code_path to enumerate the full list before changing the displacement behavior.
- **Hands-carry event vs poll.** v1 polls `HandsController.CarriedItem` at unscaled-time 4 Hz because there's no `OnCarriedItemChanged` event today. The cheap alternative is to add such an event on `HandsController` (one field, one callback fire in `CarryItem` / `DropCarriedItem`) and have the UI subscribe instead of polling. Decision deferred to plan-phase. If the event is added, the rule #34 hot-path-log gate on the poll goes away entirely; if not, the poll's log must be gated.
...
| Late-joiner: host equips a full loadout before client joins, then host re-opens its own window after client connect | Host's window still paints the host's loadout correctly post-connect — verifies `OnNetworkSpawn` / `FullSyncFromNetwork` doesn't regress host-side equipment when a client connects. |
