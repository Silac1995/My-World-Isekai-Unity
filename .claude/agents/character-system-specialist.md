...
description: "Expert in the full Character system — the Character.cs facade, capability registry, CharacterArchetype SO blueprints, CharacterSystem base class, visual abstraction (ICharacterVisual, IAnimationLayering, ICharacterPartCustomization, IBoneAttachment), CharacterActions lifecycle (timed CharacterAction + CharacterAction_Continuous condition-terminated base added 2026-05-06 with OnTick contract and ActionContinuousTickRoutine dispatcher; concrete continuous actions: CharacterAction_FinishConstruction + CharacterAction_OccupyFurniture added 2026-05-14), PlayerController/NPCController switching, IsFree availability, CharacterNeeds, CharacterStats, SkillId enum + Character.GetSkillLevelOrZero stub, OccupyingFurniture replication via NetworkOccupyingFurnitureNetId (2026-05-14), interaction providers, save/load contracts, and adding new archetypes or capabilities. Use when implementing, debugging, or designing anything related to character architecture, archetypes, capabilities, visuals, controllers, or actions (including continuous condition-terminated actions like CharacterAction_FinishConstruction / CharacterAction_OccupyFurniture)."
model: opus
color: green
memory: project
...
- **`CharacterAction_OccupyFurniture(actor, OccupiableFurniture)`** (2026-05-14) — concrete `CharacterAction_Continuous` that seats a character on furniture and holds the seat until cancelled. Single source of truth for occupy/leave across player E-press and NPC `JobVendor.Execute` paths (rule #22 player↔NPC parity). Server-only. 1 Hz. `OnStart` → `target.Use(character)` (sets `_seatingFailed` on race); `OnTick` → validate `target.Occupant == character`, call `target.Leave(character)` defensively + return true on invalidation; `OnCancel` → `target.Leave(character)` (idempotent — Leave returns false if not the current occupant). `IsReplicatedInternally = false` (standard visual proxy). `ShouldPlayGenericActionAnimation = false` (character idles at the StandingPoint, no `isDoingAction` trigger). Replaces the deleted `Cashier.ServerTickAutoOccupy` proximity tick. See `wiki/systems/shop-vendor.md` change log + `docs/superpowers/specs/2026-05-14-furniture-occupancy-via-characteraction-design.md`.
...
| `Assets/Scripts/Character/CharacterActions/CharacterAction_OccupyFurniture.cs` | Concrete continuous action — furniture occupy/leave (added 2026-05-14) |
...
- **2026-05-14 — Furniture occupancy via CharacterAction** (spec: `docs/superpowers/specs/2026-05-14-furniture-occupancy-via-characteraction-design.md`, plan: `docs/superpowers/plans/2026-05-14-furniture-occupancy-via-characteraction.md`, audit: `docs/superpowers/audits/2026-05-14-furniture-occupancy-rule-19b.md`):
  - **New continuous action**: `CharacterAction_OccupyFurniture(Character, OccupiableFurniture)`. Server-only, 1 Hz. `OnStart → Use`; `OnTick → validate, Leave + return true on invalidation`; `OnCancel → Leave` (idempotent). `IsReplicatedInternally = false` (standard 600s-sentinel visual proxy). `ShouldPlayGenericActionAnimation = false`. Same action queued from player + NPC paths — controller swaps no-op for seating state (rule #22 player↔NPC parity).
  - **`Character.OccupyingFurniture` is now replicated**. New `NetworkVariable<ulong> NetworkOccupyingFurnitureNetId` (EveryoneRead / ServerWrite). Server-write inside `SetOccupyingFurniture`; getter resolves through `NetworkManager.SpawnManager` on clients (`GetComponent<Furniture>()` fast-path for self-attached NOs like Cashier; `GetComponentInChildren<Furniture>()` fallback for furniture under a building NO). `OnValueChanged` fires `OnOccupyingFurnitureChanged` on remote peers. Fixes the rule #19b host-only-state gap that would otherwise have made the spec's literal "OccupyingFurniture != null ⇒ no movement" gate server-only-effective.
  - **Two new ServerRpcs on `CharacterActions`**: `RequestOccupyFurnitureServerRpc(NetworkBehaviourReference, Vector3)` (the Vector3 is a position tie-breaker for multi-furniture-per-building cases — Bed/Chair have no per-furniture NO, so the NB resolves to the parent building and children are walked with `FindClosestOccupiableUnder`). `RequestLeaveOccupiedFurnitureServerRpc()` — server-side `ClearCurrentAction`, lets `OnCancel`'s `Leave` release the seat.
  - **New `OccupiableFurniture.IsCharacterAllowedToOccupy(Character)` virtual** — default `true`. `Cashier` overrides to require the assigned `JobVendor` for the linked shop when `RequiresVendor`. Server-side gate (called from `CanExecute` + the ServerRpc handlers + `RequestUseCashierServerRpc`). Required because `CharacterJob._activeJobs` is not NetVar-replicated.
  - **New `CashierNetSync.RequestUseCashierServerRpc(NetworkBehaviourReference customerRef)`** — unified player E-press intent. Server validates ownership + proximity, then role-routes: assigned vendor + seat free → `CharacterAction_OccupyFurniture`; otherwise → `CharacterAction_BuyFromShop`. Targeted toasts ("No vendor on duty" / "busy") sent back via existing `ToastClientRpc` / `SendBusyToastClientRpc`. Required so remote-client player-vendors can still take their cashier without local CharacterJob reads.
  - **Default `OccupiableFurniture.OnInteract` rewritten**: server path queues directly via `ExecuteAction`; client-owner relays via the new ServerRpc. `ChairFurniture` + any future Occupiable inherits.
  - **`CashierInteractable.Interact` collapsed to two branches**: seated occupant → leave; everyone else → `RequestUseCashierServerRpc` (server decides). Local "busy" toast pre-gate kept for snappy UX (CurrentCustomer is NetVar-resolved).
  - **`JobVendor` step 3 (new)**: reserved cashier + worker in interaction zone + no current action → server-side `ExecuteAction(new CharacterAction_OccupyFurniture(_worker, _heldCashier))`. `Unassign` routes the seated case through `ClearCurrentAction` (OnCancel → Leave) + defensive direct `Leave` belt-and-suspenders.
  - **Movement gates**: `PlayerController.Move` / `CharacterMovement.SetDestination` / `CharacterMovement.SetDesiredDirection` all early-return on `_character.OccupyingFurniture != null`. Works on every peer thanks to the new NetVar replication.
  - **Deletions**: `Cashier.Update`, `Cashier.ServerTickAutoOccupy`, `_autoSeatTimer`, `AUTO_SEAT_TICK_INTERVAL`, `_scratchColliders`, `_autoSeatRadius` (incl. the orphan serialized field in `Cashier.prefab`).
  - **Known scope limits** (documented in audit): `CharacterJob._activeJobs` NetVar replication remains a separate task. Bed/Chair identity on clients is non-null-only (no per-furniture NO).
