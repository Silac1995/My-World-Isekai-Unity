...
description: "Expert in NPC autonomous behavior — Behaviour Tree priority system (with 2026-05-18 BTAction_PursueAmbition inserted between Schedule and GOAP, driving the city-founding ambition chain: reads CharacterAmbition.Current.CurrentStepQuest, dispatches the passive tasks Task_PlaceBuilding/Task_FinishConstruction/Task_PromoteCommunity to canonical paths BuildingPlacementManager.PlaceCivicBuildingForLeader + CharacterAction_FinishConstruction + Community.TryPromoteLevel; the Finalize driver runs a 6-step autonomous-gather state machine — carry-to-zone-then-drop / pickup-loose-WorldItem-near-actor / harvest-cached-target / scan-CharacterAwareness.GetVisibleInteractables<Harvestable>-for-yielder-of-missing-material / consume-zone-WorldItems / wander-NavMesh.SamplePosition-within-~18u-to-expand-awareness — chaining the canonical CharacterHarvestAction + CharacterPickUpItem + CharacterDropItem + CharacterAction_FinishConstruction so non-empty ConstructionRequirements get fulfilled autonomously; pumps AmbitionQuest.TickActiveTasks otherwise), GOAP backward-search planner, CharacterNeeds provider pattern, CharacterSchedule time slots, job system (CharacterJob + CommercialBuilding + work phases), logistics cycle (incl. furniture-first deposit + pickup paths), and all 20 GOAP actions. Use when implementing, debugging, or designing anything related to NPC decision-making, AI behavior, needs, schedules, jobs, GOAP goals, or autonomous pursuit of CharacterAmbition life-goals."
model: opus
color: green
memory: project
...
| `GoapAction_TakeMaterialFromABStorage` | **JobBuilder** fetch. Walks every `StorageFurniture` in the `AdministrativeBuilding`'s transform tree for any slot holding the next missing material from `_ab.LogisticsManager.GetFirstActiveBuildOrder()`. Mirrors `GoapAction_FetchSeed` verbatim — `IsCharacterInInteractionZone` gate (rule #36) + softlock guard + path-loss re-fire; `storage.RemoveItem` + `CharacterEquipment.CarryItemInHand`. Cost = 1.0. Preconditions: `hasMaterialsInHand=false, hasActiveBuildOrder=true, hasMatchingMaterialInABStorage=true`. Effects: `hasMaterialsInHand=true`. |
| `GoapAction_GoToConstructionSite` | **JobBuilder** move. Pure walk into `BuildOrder.TargetBuilding.BuildingZone.bounds`. The construction zone is a plain `BoxCollider` trigger (no `InteractableObject`), so arrival is 2D X-Z bounds containment — mirror of `CharacterAction_FinishConstruction.IsActorInsideBuildingZone`. Softlock guard + `!HasPath` re-fire. Cost = 0.5 (cheap). Preconditions: `hasMaterialsInHand=true, hasActiveBuildOrder=true`. Effects: `insideConstructionSite=true`. |
| `GoapAction_DropMaterialAtZone` | **JobBuilder** delivery. Once inside the zone, queues `CharacterDropItem(worker, carried)` so the carried instance becomes a loose `WorldItem` inside the zone, where `CharacterAction_FinishConstruction.ConsumeFromZone` despawns it next tick. `_actionStarted` lock + `OnActionFinished` callback sets `_isComplete` (mirrors `GoapAction_GatherStorageItems` DroppingOff pattern). Cost = 0.5. Preconditions: `hasMaterialsInHand=true, insideConstructionSite=true`. Effects: `materialDelivered=true`. |
| `GoapAction_FinishBuildingConstruction` | **JobBuilder** terminator. Wraps `CharacterAction_FinishConstruction` (Phase 1 continuous action). Subscribes to `OnActionFinished` for the wait; the continuous action exits on (a) progress hits 1.0 → `Building.Finalize`, (b) worker leaves zone, (c) MaxStallTicks of zero-consume. All three drop back to `JobBuilder.PlanNextActions` for the next trip via force-replan. Cost = 1.0. Preconditions: `insideConstructionSite=true, materialDelivered=true`. Effects: `isIdling=true` (multi-trip handled by force-replan on `IsComplete`). |
...
**Job variants**: `JobCrafter` (skill prerequisites), `JobTransporter` (internal GOAP planner), `JobLogisticsManager` (order fulfillment), `JobFarmer` (cell-targeted plant/water cycle on `FarmingBuilding`), `JobBuilder` (city construction; consumes `BuildOrder` on `AdministrativeBuilding` via the four NEW JobBuilder GOAP actions; mirrors JobFarmer's planner shape verbatim — see `wiki/systems/job-builder.md`), `JobHarvester` (also runs a **CityHarvester runtime branch** when workplace is `AdministrativeBuilding` — 7-state manual state machine drains `ab.GetUnfulfillableHarvestQueue()` via `Physics.OverlapSphereNonAlloc` scan + `CharacterHarvestAction` + `CharacterPickUpItem` + `CharacterStoreInFurnitureAction`; the existing GOAP actions take `HarvestingBuilding` and can't be reused for AB)
...
- **2026-04-25 → 2026-04-27 — NPC AI performance pass (Tier 1 + Tier 3, hit 60 FPS target):**
  Patterns now canonical for any NPC / job / GOAP work — see [[performance-conventions]] for the full catalogue. Active TODOs in [[optimisation-backlog]].
  - **`CharacterAwareness` 0.3 s TTL cache** ([`CharacterAwareness.cs`](../../Assets/Scripts/Character/CharacterAwareness.cs)): every `GetVisibleInteractables()` call used to allocate a fresh `List<InteractableObject>` + a `Collider[]` from `Physics.OverlapSphere` AND fired `Debug.Log` on every successful typed call (host-progressive-freeze pattern). Now: shared cached list + reused `Collider[64]` `OverlapSphereNonAlloc` buffer + 0.3 s TTL. The untyped overload returns a SHARED reference — callers must treat it as read-only. Public `InvalidateCache()` for callers needing immediate freshness. **All BT conditions (`BTCond_DetectedEnemy`, `BTCond_FriendInDanger`, `BTCond_WantsToSocialize`) automatically benefit; combat reaction latency 0.1 s → ≤ 0.3 s, well under human reaction.**
  - **`Job.ExecuteIntervalSeconds` cadence stagger** ([`Job.cs`](../../Assets/Scripts/World/Jobs/Job.cs) + [`BTAction_Work.HandleWorking`](../../Assets/Scripts/AI/Actions/BTAction_Work.cs)): default `0.1f` (10 Hz, BT tick rate). `JobLogisticsManager` and `JobHarvester` override to `0.3f` (3.3 Hz). The BT itself still ticks at 10 Hz — only `Job.Execute` is throttled. Combat reaction, schedule transitions, and action lifecycle (move/interact) are NOT slowed. **`UnityEngine.Time.time` must be fully-qualified** because of the `MWI.Time` namespace clash (caught one transient compile error during the pass — don't repeat).
  - **`GoapAction_GatherStorageItems` per-action cache + `OverlapBoxNonAlloc`** ([`GoapAction_GatherStorageItems.cs`](../../Assets/Scripts/AI/GOAP/Actions/GoapAction_GatherStorageItems.cs)): 3 per-call `Physics.OverlapBox` (BuildingZone, DepositZone, DeliveryZone) replaced with `Physics.OverlapBoxNonAlloc` against a static shared `Collider[128]` buffer + reused scratch `List<Collider>`. Per-action 0.5 s TTL cache for `FindLooseWorldItem` result (cleared in `Exit()` so a new action invocation starts cold; cache invalidated if cached item gets picked up). **Pattern reusable for any GOAP action whose `IsValid` does heavy `Physics.Overlap*`** — see [[performance-conventions]] Pattern 6.
  - **GOAP action lifecycle still uses fresh-per-plan instances** in `JobTransporter`, `JobHarvester`, `JobLogisticsManager`. **DO NOT pool `GoapAction` instances without profiler evidence** — the `JobLogisticsManager.cs:173-178` comment block memorializes a prior regression where this exact change broke shop ordering (`_isComplete=true` leaked across plans, only first BuyOrder placed). Tier 4 `Dₐ` deferral; the audit explicitly said "do this last with profiler diff."
  - **Network safety:** every cache and dirty flag is server-only state. `Job.Execute` runs server-side. `BTAction_Work` runs server-side. `CharacterAwareness` runs per-peer. None of these touch `NetworkVariable` / `NetworkList`.
  - **Tier 4 deferred (NOT yet shipped, capture in [[optimisation-backlog]] before any new perf work):**
    - `CharacterActions.ActionTimerRoutine` `Instantiate` pooling (~0.8 MB / frame). The action timer coroutine instantiates fresh prefabs on `OnApplyEffect` for `CharacterDropItem` / `CharacterPickUpItem` / `CharacterStoreInFurnitureAction` / `CharacterPlaceFurnitureAction` / `CharacterCraftAction`. Network-aware `WorldItem` pool needed — see `network-architecture` § NGO Pooling.
    - In-coroutine `Debug.Log` inside `ActionTimerRoutine` (3 calls / frame, 11.7 KB / frame from `StackTraceUtility.ExtractStackTrace`). Gate behind `NPCDebug.VerboseActions`. Free fix while doing the pooling work.
    - GOAP planner allocations (action list ctor per replan in JobTransporter / JobHarvester / JobLogisticsManager) — Tier 4 `Dₐ`, do not ship without profiler evidence.
...
- **Performance Conventions**: [`wiki/concepts/performance-conventions.md`](../../wiki/concepts/performance-conventions.md) — pattern catalogue (TTL cache, NonAlloc, ExecuteIntervalSeconds cadence, per-action TTL). Read before any per-tick / per-NPC work.
