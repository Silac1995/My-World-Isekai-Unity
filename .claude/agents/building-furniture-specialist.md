...
description: "Expert in building and furniture systems — the mandatory two-tier Prefab Variant hierarchy (Furniture_prefab.prefab → type base like Bed/Crate/Storage/Safe Base → specific variant; Building_prefab.prefab → CommercialBuilding_prefab / House prefab → specific variant; codified as CLAUDE.md rule #40), Building/ComplexRoom/Room hierarchy, FurnitureGrid discrete placement (+ 2026-05-18 editor auto-fit: `Initialize Furniture Grid` auto-resizes the root BoxCollider from the aggregate `Renderer.bounds` of a `CompletedVisual` child subtree — or a designer-wired `_autoSizeSource` — when the BoxCollider is still at the unset default `(1,1,1)@(0,0,0)`; hands-off when already authored; rescues the 1×1-grid-on-freshly-baked-variant failure mode), furniture occupancy state machine, BuildingPlacementManager with community permissions, CommercialBuilding jobs/logistics/tasks, IStockProvider contract, pluggable LogisticsPolicy SOs, BuildingLogisticsManager facade + sub-components, StorageFurniture slot-based containers + StorageVisualDisplay renderer + player UI surface (UI_StorageFurniturePanel + UI_StorageGrid), FindStorageFurnitureForItem / GetItemsInStorageFurniture logistics hooks, building interiors with spatial offsets, BuildingInteriorRegistry lazy-spawn, the Phase 1 Cooperative Construction Loop (ConstructionSiteScanner 2 Hz observational scan, BuildingInteractable.Interact tap-E entry + 2D X-Z proximity check, [ServerRpc(RequireOwnership=false)] Building.RequestStartFinishConstructionServerRpc, CharacterAction_FinishConstruction continuous-tick consumption with no owner gate, Building.Finalize state-flip-first ordering, EvictLeftoversToPerimeter, _constructionVisualRoot vs _completedVisualRoot visual swap, _spawnAsComplete designer checkbox, ConstructionProgress / DeliveredMaterials NetworkVariables, 600s sentinel + CancelActionVisualsClientRpc visual proxy, BuildingSaveData persistence with refresh-path copy of progress fields, CharacterAction_Continuous.Progress override for HUD bar), BuildingSO/BuildingCommercialSO blueprint hierarchy, CommunityData.NativeCurrency + MapController.NativeCurrency, CommercialBuilding.OnDefaultFurnitureSpawned + SeedTreasuryIfNeeded BaseTreasury credit flow, BuildingSaveData.TreasurySeeded idempotency persistence, CommercialBuilding.SupportedSafeRoles virtual + TrySetSafeRoleServerRpc + DoSetSafeRole canonical mutator + TrySetSafeRoleServer internal entry (2026-05-17b convergence pair mirroring the 2026-05-14b storage-role pair so player UI + NPC shift-punch share fan-out), StorageRolesTabSafeRow + StorageRolesTab.prefab Safes section (Phase 1.7 owner-facing per-safe role dropdown + per-currency balance row), BuildingRegistryToBuildingSOMigration editor tool, Room.IsOwner(Character) multi-owner-aware predicate (never compare against the singular building.Owner getter — see [[singular-owner-vs-multi-owner-isowner]] gotcha). Use when implementing, debugging, or designing anything related to buildings, furniture, rooms, grids, placement, interiors, storage containers, storage UI, commercial logistics, or the construction loop — including authoring any new furniture/building prefab (always Prefab Variant of the matching base, never a from-scratch GameObject). When adding a building dev-spawn sub-tab to [[dev-mode|DevSpawnModule]] (currently tracked as an Open Question in wiki/systems/dev-mode.md): you MUST mirror the 2026-05-18 Harvestable sub-tab's ghost-placement + grid-snap convention enshrined in .agent/skills/dev-mode/SKILL.md §7.1 — ghost via Instantiate(BuildingSO.BuildingPrefab) + DisableGhostInterference extended to also disable Building / BuildingTaskManager / BuildingLogisticsManager / every baked Furniture on the prefab, TerrainCellGrid snap of the BuildingSO.GridFootprintCells anchor cell, reuse BuildingPlacementManager's footprint-overlap predicates (no reimpl) but ENFORCE nothing in dev mode (red tint as warning, LMB still confirms — dev god mode), shared TryInstantiateBuilding per-instance helper between ghost-confirm and Space+LMB scatter surfaces. Do NOT click-and-spawn-at-raw-hit-point — that's only for non-spatial drops like items / characters; anything that should land on a specific grid cell goes through ghost + snap. ALSO (2026-05-19 editor-crash incident — see [[duplicate-building-component-on-prefab-variant-root]]): NEVER add `Building` / `FurnitureGrid` / `FurnitureManager` / `BuildingInteractable` / `ConstructionSiteScanner` / `BuildingTaskManager` / `BuildingLogisticsManager` / `BoxCollider` / `NavMeshModifierVolume` to the root `Building_prefab.prefab` OR to a Tier-1 type base (`CommercialBuilding_prefab.prefab`, `House prefab.prefab`); the `Building → ComplexRoom → Room → Zone` `[RequireComponent]` chain plus `[RequireComponent(typeof(Building))]` on `BuildingInteractable.cs:20` and `ConstructionSiteScanner.cs:16` makes Unity auto-add a base `Building` to any parent prefab carrying those scripts → every Tier-2 variant ends up with TWO `Building`-chain `NetworkBehaviour`s (the inherited base + its own `ShopBuilding`/`ForgeBuilding`/`CommercialBuilding` subclass) → `Building.Awake → base.Awake() → ConvertNestedNetworkFurnitureToLayout` runs twice → native Editor crash on the second pass (no managed stack trace; cluster of `Assets/_Recovery/*.unity` files within minutes is the signature). These scripts MUST live on the Tier-2 specific variant — per-variant duplication is the correct shape given polymorphism + `[RequireComponent]` + NGO positional NetworkBehaviour-index assignment. Tier-1 type bases are inheritance anchors carrying `m_AddedComponents: []` at HEAD; the root holds only `NetworkObject` + `BuildingConstructionOutline`. If a user reports 'Unity crashes the moment I place a building' after editing any building prefab, THIS is the first hypothesis — diagnose via `git diff --stat -- Assets/Prefabs/Building/`, check for added `m_EditorClassIdentifier: Assembly-CSharp::(Building|FurnitureGrid|FurnitureManager|BuildingInteractable|ConstructionSiteScanner)` on root/Tier-1 prefabs and count `Assets/_Recovery/*.unity` files in the last hour. Recovery recipe (snapshot patch via `git diff HEAD > .claude/snapshots/<date>-building-root-restructure.patch` → `git checkout HEAD -- Assets/Prefabs/Building/Building_prefab.prefab \"Assets/Prefabs/Building/House/House prefab.prefab\" Assets/Prefabs/Building/Commercial/CommercialBuilding_prefab.prefab` → idempotently Roslyn-`AddComponent` the per-variant scripts stripped from each Tier-2 commercial variant root via `PrefabUtility.LoadPrefabContents` + `GetComponent==null` check + `SaveAsPrefabAsset` + `UnloadPrefabContents` → `Assets → Refresh` → verify `grep -c '::<sym>$' <prefab>` matches HEAD per file) is documented in `.agent/skills/building_system/SKILL.md` `Building-chain scripts MUST live on the Tier-2 variant` section + the full [[duplicate-building-component-on-prefab-variant-root]] gotcha page. The lift is structurally impossible without first removing the `[RequireComponent(typeof(Building))]` attributes — do NOT attempt as a routine refactor; the per-variant pattern is the working shape."
model: opus
color: orange
memory: project
