...
| `BuildingInspectorView` | `IBuildingInspectorView` for any `Building` subtype. Single-view runtime polymorphism — renders Identity (concrete type, BuildingName, BuildingId, PrefabId, IsPublicLocation, PlacedByCharacterId) → State (CurrentState + per-material `required / contributed / pending` from `Building.ConstructionRequirements` / `ContributedMaterials` / `GetPendingMaterials()`) → Owners (resolved from inherited `Room.OwnerIds`) → Commercial-only block (Operational, OwnerCommunity, InventoryTotalCount, Jobs each with `Job.CurrentGoalName` + `Job.CurrentActionName` per assigned worker, ActiveWorkersOnShift roster, TimeClock present/absent) → Wanted Resources (HarvestingBuilding only — authored `WantedResources` config with current stock vs cap) → Tracked Harvestables (HarvestingBuilding only — every node from `HarvestingBuilding.TrackedHarvestables`: scanned-zone label, alive/depleted summary, per-Item alive yield aggregate via `Harvestable.HarvestOutputs × RemainingYield`, then per-node row with state/yield/category/cell coupling) → Needed Resources (any commercial — Harvest backlog from `HarvestingBuilding.GetWantedItems()` AND Crafting deficit synthesized by expanding `ActiveCraftingOrders` recipes × remaining quantity, aggregated per ItemSO, then `need - GetItemCount = deficit`) → Logistics Orders (active + placed BuyOrders via `BuildingLogisticsManager.ActiveOrders` / `PlacedBuyOrders`, ActiveCraftingOrders with per-crafter contribution map, active + placed TransportOrders with carrier name from `IQuest.Contributors`) → Tasks (Available + In-Progress via `BuildingTaskManager.AvailableTasks` / `InProgressTasks`, each row showing Title → Target.gameObject.name and the `ClaimedByWorkers` roster) → Rooms (per-room furniture count) → Furniture (flat list with `[Room] FurnitureName (Type)`) → Interior (`SupportsInterior` authored flag + `HasInterior` spawned flag + `GetInteriorMapId`). Refresh-every-frame; cheap because `DevInspectModule` keeps non-active views disabled. | `Assets/Scripts/Debug/DevMode/Inspect/BuildingInspectorView.cs` |
