...
description: "Expert in debug/dev tools infrastructure — the Dev-Mode god tool (DevModeManager owning global shortcuts Ctrl+Click interior-select / Alt+Click building-select / Space+LMB spawn / ESC cancel, DevModePanel, DevSpawnModule Spawn tab with three sub-tabs Character / Item / Harvestable + the canonical ghost-placement-with-grid-snap pattern enshrined 2026-05-18 for the Harvestable sub-tab (Instantiate(so.HarvestablePrefab) ghost → DisableGhostInterference strip NetworkObject + Rigidbody + Collider + NavMeshObstacle + Harvestable + HarvestableNetSync + Ignore-Raycast layer → Update loop raycasts cursor on Environment layer → MapController.GetMapAtPosition → TerrainCellGrid.WorldToGrid → GridToWorld snap + preserve raycast hit Y → green-on-grid / yellow-off-grid tint via TintGhost SpriteRenderer.color + MeshRenderer.material.color → LMB confirm via ConfirmHarvestableGhostSpawn + chain-rebuild → RMB/ESC/disarm/sub-tab-change/dropdown-change/dev-mode-off all clear → Space+LMB scatter still works with per-instance grid snap → shared TryInstantiateHarvestable per-instance helper between ghost-confirm + scatter surfaces). Future placement-aware sub-tabs (building spawn, furniture-in-world spawn, NPC-in-formation spawn, …) MUST follow this same ghost-placement + grid-snap convention — see .agent/skills/dev-mode/SKILL.md \"Placement-aware sub-tab pattern\" section. DevSelectionModule + IDevAction Select tab generalized to InteractableObject with dual interior + building masks AND a separate SelectedBuilding slot for raw Building targets, Inspect tab with DevInspectModule routing both IInspectorView + IBuildingInspectorView, CharacterInspectorView + 11 CharacterSubTab categories (10 text-based via the abstract RenderContent contract + the widget-based CharacterCityFoundingSubTab added 2026-05-18 — the latter overrides the now-virtual base Refresh to render programmatic UGUI widgets directly; parents under a SerializeField _widgetRoot RectTransform; sections: DEV banner / Refresh / Status header / Create Community / Assign Ambition_FoundACity / Community readout / Force-Promote / Grant Treasury / Submit Join Request / Time control with Force NewDay), StorageFurnitureInspectorView for chest/shelf/barrel/wardrobe slot listings with 'Inspect Parent Building' navigation hook, BuildingInspectorView with a 2-sub-tab structure (Overview = full read-out via BuildingOverviewSubTab + [DEV] Console Management = mutator surface via BuildingConsoleManagementSubTab) backed by a BuildingSubTab base class mirroring CharacterSubTab, plus host-only DevForce* server methods on CommercialBuilding (DevForceSetHiring / DevForceSetAssignmentWage / DevForceSetStorageRole / DevForceSetSafeRole / DevForceCreditTreasury), AdministrativeBuilding (DevForceChangeCommunityLevel), TimeManager (DevForceNewDay), and ShopBuilding (DevForceAdd/Remove/EditCatalogEntry / DevForceWithdrawCashierTill) that bypass owner / community-leader auth gates — wrapped in #if UNITY_EDITOR || DEVELOPMENT_BUILD, gated by DevAssertHostAndDevMode (IsServer + DevModeManager.IsEnabled + audit log line) or an equivalent inline IsServer+IsEnabled check on non-NetworkBehaviour singletons like TimeManager, routed through Do* helpers extracted from the production ServerRpc bodies so production replication paths are bit-for-bit identical, HarvestableInspectorView for any Harvestable (wilderness or crop) — Identity / Harvestable state / Destruction / Crop sections with live CropHarvestableNetSync NetVars + full TerrainCell readout, CharacterAIDebugFormatter, DevInspectTabBuilder + DevStorageFurnitureInspectorBuilder + DevHarvestableInspectorBuilder Editor scripts, /devmode chat command), DebugScript spawning UI, MapControllerDebugUI hibernation diagnostics, UI_CharacterDebugScript NPC state visualization, UI_CommercialBuildingDebugScript logistics display, and creating new debug panels, cheat commands, diagnostic overlays, and inspection views. Use when creating, extending, or improving debug tools."
model: opus
memory: project
...
| `BuildingInspectorView` | `IBuildingInspectorView` for any `Building` subtype. As of 2026-05-09 hosts a 2-sub-tab structure (mirror of `CharacterInspectorView`): **Overview** (read-only — handed to `BuildingOverviewSubTab`) + **[DEV] Console Management** (mutator surface — handed to `BuildingConsoleManagementSubTab`). Tab bar built programmatically at runtime (no prefab edits, no Editor builder needed for 2 sub-tabs); the existing `_headerLabel` / `_content` SerializeFields stay wired and `_content` is re-parented under the Overview sub-tab's host on Awake. **Layout strategy** (commit `ef136b57`): hides every legacy prefab child that isn't a serialized field reference (the prefab's pre-existing `Slots` wrapper would otherwise hog vertical space via `LayoutElement(min=200, flex=10)`), then bypasses the prefab's outer `VerticalLayoutGroup` for the runtime hosts via `LayoutElement.ignoreLayout = true` + manual stretch anchors (helper `ConfigureContentHostStretch`). See `wiki/gotchas/runtime-uichildren-collapse-under-prefab-vlg.md` for the full diagnosis. Refresh-every-frame; only the active sub-tab's `Refresh(Building)` is dispatched. The Console Management tab button is tinted red so it's impossible to confuse with a production surface. | `Assets/Scripts/Debug/DevMode/Inspect/BuildingInspectorView.cs` |
| `BuildingSubTab` | Abstract base for one category of the Building inspector (mirror of `CharacterSubTab`). Subclasses override `DoRefresh(Building)` and `DoClear()`; the base wraps both in try/catch so a thrown sub-tab never wipes the dev panel. | `Assets/Scripts/Debug/DevMode/Inspect/SubTabs/BuildingSubTab.cs` |
| `BuildingOverviewSubTab` | Read-only Overview sub-tab. Verbatim port of the prior `BuildingInspectorView.RenderContent` — Identity / State / Owners / Commercial / Inventory / Wanted Resources / Tracked Harvestables / Needed Resources / Logistics Orders / Tasks / Rooms / Furniture / Interior, all into the same TMP_Text the prefab serializes. Behavior unchanged from the pre-2026-05-09 view. | `Assets/Scripts/Debug/DevMode/Inspect/SubTabs/BuildingOverviewSubTab.cs` |
| `CharacterCityFoundingSubTab` | DEV-ONLY widget-based 11th sub-tab on `CharacterInspectorView` ("Founding"). Drives the Plan 4 city-founding loop end-to-end without authoring content or waiting for the natural in-game cadence. Inherits `CharacterSubTab` but overrides the now-virtual base `Refresh(Character)` to render programmatic UGUI widgets directly (matching `BuildingConsoleManagementSubTab`); `RenderContent` returns empty to satisfy the abstract contract. Sections (target-state-gated): DEV banner / Refresh / Status header / Create Community (when `CurrentCommunity == null`) / Assign Ambition_FoundACity / Community readout / Force-Promote (`±1 tier` via `AdministrativeBuilding.DevForceChangeCommunityLevel`) / Grant Treasury (`DevForceCreditTreasury(amount)` — currency = enclosing map's NativeCurrency) / Submit Join Request (when not in a community + ≥1 chartered AB in scene; routes through `AdministrativeBuilding.SubmitJoinRequestServerRpc` identically to the drifter ↔ JoinRequestDesk path) / Time control (live Day / Hour / Phase + `TimeManager.DevForceNewDay(count)`). Widgets parent under a `[SerializeField] private RectTransform _widgetRoot` (wired in prefab to inner `Viewport/Content` with VLG + ContentSizeFitter); falls back to `transform` when null. Mutators capture `_bound` into locals before passing into lambdas (avoid closure-over-mutable-field race when sibling button clicks rebuild). | `Assets/Scripts/Debug/DevMode/Inspect/SubTabs/CharacterCityFoundingSubTab.cs` |
| `AdministrativeBuilding.DevForce*` | Host-only dev-override on `AdministrativeBuilding`: `DevForceChangeCommunityLevel(int delta)` (2026-05-18). Clamps `OwnerCommunity.level` by `delta` tiers (bounded to the `CommunityLevel` enum range), then calls `Community.ChangeLevel((CommunityLevel)clamped)` — bypassing `Community.TryPromoteLevel`'s population / treasury / required-building gates. Wrapped in `#if UNITY_EDITOR \|\| DEVELOPMENT_BUILD` + gated by the inherited `DevAssertHostAndDevMode` helper. | `Assets/Scripts/World/Buildings/CommercialBuildings/AdministrativeBuilding.cs` |
| `CommercialBuilding.DevForceCreditTreasury` | Host-only dev-override on `CommercialBuilding`: `DevForceCreditTreasury(int amount)` (2026-05-18). Resolves currency from the enclosing map's `MapController.NativeCurrency` (CurrencyId.Default fallback when no map encloses — mirrors `SeedTreasuryIfNeeded`'s resolution), then routes through the canonical `CreditTreasury(currency, amount, reason)` path. No-op on non-positive amounts and when the building has no Treasury-role safe (CreditTreasury logs + skips). | `Assets/Scripts/World/Buildings/CommercialBuilding.cs` |
| `TimeManager.DevForceNewDay` | Host-only dev-override on `MWI.Time.TimeManager` (NOT a NetworkBehaviour): `DevForceNewDay(int count = 1)` (2026-05-18). Bumps `CurrentDay` by `count` and fires `OnNewDay` once per skipped day so server subscribers (`DrifterMigrationSystem`, `FarmGrowthSystem`, community trackers, ambition deadlines) see the same event volume as a natural midnight rollover. Phase/hour are left untouched — `TimeSkipController` is the right path for natural-speed hour-by-hour advancement. Gated inline: `NetworkManager.Singleton.IsServer` (when listening) + `DevModeManager.Instance.IsEnabled` (TimeManager has no inherited `DevAssertHostAndDevMode`). | `Assets/Scripts/DayNightCycle/TimeManager.cs` |
| `BuildingConsoleManagementSubTab` | DEV-ONLY mutator surface. Builds widgets at runtime (programmatic UGUI) — DEV banner + Construction force-finalize (when under construction) + Hiring toggle + Owner add/remove/transfer/clear + per-Job force-hire/force-fire + Storage Roles (per-storage button row driven by `CommercialBuilding.SupportedStorageRoles` — None / Tool / Inventory / SellShelf for Shop) + **Safes** (2026-05-17 — per-safe row showing current role + per-currency balance summary, button per `SupportedSafeRoles` entry, aggregate-treasury header in `CurrencyId.Default`; fires `DevForceSetSafeRole`) + Catalog add/remove (Shop) + Cashier withdraw → host (Shop) + Inventory add/remove by `ItemId` (uses `ItemSO.CreateInstance()` factory). All actions are gated `IsServer` and call either (a) server-only methods that already lack auth (`SetOwner`, `AddOwner`, `RemoveOwner`, `AssignWorker`, `RemoveWorker`, `Finalize`, `AddToInventory`, `TakeFromInventory`) or (b) the new `DevForce*` server methods on `CommercialBuilding`/`ShopBuilding` that bypass the owner / community-leader auth gate. Rebuilds widgets on bind change; widget callbacks call `RebuildAll()` so the panel reflects the post-action state immediately. | `Assets/Scripts/Debug/DevMode/Inspect/SubTabs/BuildingConsoleManagementSubTab.cs` |
| `CommercialBuilding.DevForce*` | Host-only dev-override server methods on `CommercialBuilding`: `DevForceSetHiring(bool)`, `DevForceSetAssignmentWage(...)`, `DevForceSetStorageRole(StorageFurniture, StorageRoleType)`, `DevForceSetSafeRole(SafeFurniture, SafeRoleType)` (2026-05-17). The storage-role peer writes through the storage's sibling `StorageFurnitureNetworkSync.SetRoleServer` (same NetworkVariable fan-out as production `TrySetStorageRoleServerRpc`) and honours the building's `SupportedStorageRoles` subtype filter. The safe-role peer routes through the canonical `DoSetSafeRole` convergence helper — same fan-out as production `TrySetSafeRoleServerRpc` and the NPC shift-punch `AssignSafeRolesForShift`; honours `SupportedSafeRoles`. Each is wrapped in `#if UNITY_EDITOR \|\| DEVELOPMENT_BUILD`, gated by the `protected DevAssertHostAndDevMode(action)` helper (asserts `IsServer` + `DevModeManager.Instance.IsEnabled`, emits a `<color=magenta>[DevMode]</color>` audit log line with `BuildingId` + `LocalClientId`). No RPC, no `devOverride` parameter — dev mode is host-only by hard policy (`DevModeManager.TryEnable` rejects non-host callers), so the dev panel runs on the server and direct call is safe. Production owner-gated entry points (`TryOpenHiring` / `TryCloseHiring` / `TrySetAssignmentWage` / `TrySetStorageRoleServerRpc`) are untouched. | `Assets/Scripts/World/Buildings/CommercialBuilding.cs` |
| `ShopBuilding.DevForce*` | Host-only dev-override server methods on `ShopBuilding`: `DevForceAddCatalogEntry`, `DevForceRemoveCatalogEntry`, `DevForceEditCatalogEntry`, `DevForceWithdrawCashierTill(Cashier, recipient)`. Each calls a private `Do*` helper that was extracted from the existing `ServerRpc` body during the same refactor. Production `[ServerRpc(RequireOwnership=false)]` entry points still gate on `ValidateOwnerCaller`, then call the same `Do*` helper — production behavior is bit-for-bit identical. Dev path skips the `ValidateOwnerCaller` check; same `DevAssertHostAndDevMode` audit applies. The `DoWithdrawCashierTill` helper takes the recipient explicitly so the dev path can deposit into the host's wallet (production deposits into the calling owner). Sell-shelf toggling is no longer a Shop-specific RPC — it migrated to `CommercialBuilding.TrySetStorageRoleServerRpc` (StorageRoles refactor 2026-05-08); dev override lives on `CommercialBuilding.DevForceSetStorageRole`. | `Assets/Scripts/World/Buildings/CommercialBuildings/ShopBuilding.cs` |
...
| `CashierInspectorView` | `IInspectorView` for `Cashier` furniture. `CanInspect` matches `CashierInteractable` (dedicated InteractableObject) OR `FurnitureInteractable` whose `Furniture` is a `Cashier`. Renders Identity / Cashier state (RequiresVendor, IsOccupied, Vendor via the rule-#19-safe `Cashier.Occupant` getter, CurrentCustomer, IsAvailableForCustomer) / Till (reads replicated `CashierNetSync.TillBalances` `NetworkList` — visible on every peer; falls back to server-only `_till` dict only when NetSync not yet spawned) / Linked Building (type, name, `BuildingId`, isShop) / Network sync (`NetSync.IsSpawned`, `IsServer`, `NetworkObjectId`, `OccupantNetworkObjectId.Value`, `CurrentCustomerNetworkObjectId.Value`, `LinkedBuildingRef` resolution, replicated till count) / Active action (`NetSync.ActiveAction` mode + customer — server-only field; surfaces an explicit hint on clients so the empty section mid-transaction isn't confusing). Optional "Inspect Parent Building" nav button mirrors `StorageFurnitureInspectorView` (resolves `_target.LinkedBuilding` first, falls back to `GetComponentInParent<Building>()`). Read-only. Cashier prefabs already sit on the `Furniture` layer so no selection-mask change was needed. | `Assets/Scripts/Debug/DevMode/Inspect/CashierInspectorView.cs` |
| `DevCashierInspectorBuilder` | Editor-only one-shot. `[MenuItem("Tools/DevMode/Build Cashier Inspector")]` adds the cashier view GO to the DevModePanel prefab under `InspectContent/Views` and wires `_headerLabel` + `_content`. Optional nav-button fields (`_selectionModule`, `_inspectParentBuildingButton`, `_inspectParentBuildingLabel`) are left for manual wiring in the prefab inspector. Idempotent + destructive variants. | `Assets/Editor/DevMode/DevCashierInspectorBuilder.cs` |
...
- `bool SupportsInterior` — authored interior flag, looks up `WorldSettingsData.GetInteriorPrefab(_prefabId)` through a static `_cachedWorldSettings` (one-shot lazy load; failed loads are remembered so we don't re-probe).
- pre-existing `HasInterior` / `GetInteriorMap()` / `GetInteriorMapId()` — runtime spawned-interior accessors.

`BuildingLogisticsManager` surface used by the inspector (already public):
`ActiveOrders` / `PlacedBuyOrders` / `ActiveTransportOrders` / `PlacedTransportOrders` / `ActiveCraftingOrders`.

`BuildingTaskManager` surface used by the inspector (already public):
`AvailableTasks` / `InProgressTasks`. `BuildingTask.Target` (MonoBehaviour) and `BuildingTask.ClaimedByWorkers` give the per-row target name + claimed worker roster.

`HarvestingBuilding` surface used by the inspector (already public):
`WantedResources` (authored config) / `GetWantedItems()` (items still under cap, the live "shopping list") / inherited `GetItemCount(ItemSO)` / `TrackedHarvestables` (live read-only view of the scanned harvest nodes — added 2026-04-29 specifically for inspector use; populated by `ScanAndRegisterZone`) / `HarvestableZone` + `HarvestingAreaZone` (active vs authored scan zones). The Wanted-Resources section shows the authored config; Tracked-Harvestables walks `TrackedHarvestables` and reads each node's `IsDepleted` / `RemainingYield` / `Category` / `IsCellCoupled` / `CellX,CellZ` / `HarvestOutputs` / `SO` (for label resolution). Needed-Resources reuses `GetWantedItems()` for the harvest-backlog half. The crafting-deficit half is synthesised inline (recipe expansion × remaining order quantity, minus on-hand stock) — it does **not** subtract in-flight BuyOrders, that's the LogisticsStockEvaluator's job; the inspector is intentionally a pure read-out.
...
Assets/Scripts/Debug/DevMode/Inspect/
  ...
  HarvestableInspectorView.cs        ← wilderness + crop harvestables w/ terrain-cell read-out
  CashierInspectorView.cs            ← cashier vendor / customer / till / linked building / NetSync
...
- **`DevHarvestableInspectorBuilder.cs`** — `[MenuItem("Tools/DevMode/Build Harvestable Inspector")]`. Adds the `HarvestableInspectorView` GameObject under `InspectContent/Views`. Requires the Inspect tab to already exist.
- **`DevCashierInspectorBuilder.cs`** — `[MenuItem("Tools/DevMode/Build Cashier Inspector")]`. Adds the `CashierInspectorView` GameObject under `InspectContent/Views` and wires `_headerLabel` + `_content`. Optional nav-button fields are left for manual wiring. Requires the Inspect tab to already exist.
