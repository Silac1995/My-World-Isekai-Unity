...
- Hosts 3 always-visible identity copy buttons (added 2026-05-09) in a sibling `IdentityCopyRow` (sibling index 1, between `Header` and `TabBar`): `_copyUidButton` writes `Character.CharacterId` (the persistent profile GUID — the only identity key, what `Character.FindByUUID` resolves and `CharacterProfileSaveData.characterGuid` round-trips), `_copyWorldGuidButton` writes `Character.OriginWorldGuid` (provenance metadata — birth-world, *not* part of identity since a character can travel between worlds), `_copyAllButton` writes a `Name + UID + OriginWorld` block. All write to `GUIUtility.systemCopyBuffer` inside `try/catch` (per rule 31). `SetTarget` / `Clear` toggle `Button.interactable` so the row greys out when nothing is selected. Use these to seed `Character.FindByUUID` calls in test code, paste into save-file searches, or report bugs touching [[save-load]] / [[character-profile]].
