...
/// <summary>
/// One-shot Editor utility that adds a <see cref="StorageFurnitureInspectorView"/> GameObject
/// to the DevModePanel prefab as a sibling of the existing CharacterInspectorView. The view
/// is auto-discovered at runtime by <see cref="DevInspectModule"/> via
/// <c>GetComponentsInChildren&lt;IInspectorView&gt;(true)</c>, so no further wiring is required
/// once this builder runs.
///
/// Menu:
///   Tools/DevMode/Build Storage Furniture Inspector                 — additive, aborts if it already exists.
///   Tools/DevMode/Rebuild Storage Furniture Inspector (Destructive) — deletes existing then rebuilds.
/// </summary>
public static class DevStorageFurnitureInspectorBuilder
{
    private const string PrefabPath = "Assets/Resources/UI/DevModePanel.prefab";
    private const string ViewName = "StorageFurnitureInspectorView";

    [MenuItem("Tools/DevMode/Build Storage Furniture Inspector")]
    public static void BuildStorageInspector()
    {
        RunBuild(destructive: false);
    }

    [MenuItem("Tools/DevMode/Rebuild Storage Furniture Inspector (Destructive)")]
    public static void RebuildStorageInspector()
    {
        bool confirmed = EditorUtility.DisplayDialog(
            "Rebuild Storage Furniture Inspector",
            "This will DELETE any existing 'StorageFurnitureInspectorView' under DevModePanel/ContentRoot/InspectContent/Views, then rebuild it from scratch.\n\nAre you sure?",
            "Yes, rebuild",
            "Cancel");
        if (!confirmed) return;
    ...
    private static void RunBuild(bool destructive)
    {
        ...
        if (contents == null)
        {
            Debug.LogError($"[DevStorageFurnitureInspectorBuilder] Could not load prefab at {PrefabPath}");
            return;
        }

        try
        {
            // --- Locate the Views container --------------------------------------------
            ...
            {
                Debug.LogError("[DevStorageFurnitureInspectorBuilder] ContentRoot not found. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            var inspectContentT = contentRootT.Find("InspectContent");
            if (inspectContentT == null)
            {
                Debug.LogError("[DevStorageFurnitureInspectorBuilder] InspectContent not found under ContentRoot. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            var viewsT = inspectContentT.Find("Views");
            if (viewsT == null)
            {
                Debug.LogError("[DevStorageFurnitureInspectorBuilder] Views not found under InspectContent. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            ...
            if (existing != null)
            {
                Debug.LogWarning($"[DevStorageFurnitureInspectorBuilder] '{ViewName}' already exists. Use the destructive variant to overwrite.");
                return;
            }

            // Reference font (reuse anything already in the prefab so the new view matches the look).
            var referenceFont = FindFirstFontAsset(contents);
            if (referenceFont == null)
            {
                referenceFont = TMP_Settings.defaultFontAsset;
                Debug.LogWarning("[DevStorageFurnitureInspectorBuilder] No TMP font found in prefab; falling back to TMP_Settings.defaultFontAsset.");
            }
            ...
            GameObject scrollGO = CreateScrollRect(viewRoot.transform, "Slots", referenceFont, out TMP_Text contentTMP);
            ...
            {
                Debug.LogError("[DevStorageFurnitureInspectorBuilder] StorageFurnitureInspectorView is missing expected serialized fields (_headerLabel / _content).");
            }
            else
            {
            ...
            if (!success)
            {
                Debug.LogError("[DevStorageFurnitureInspectorBuilder] PrefabUtility.SaveAsPrefabAsset returned success=false.");
            }
            else
            {
                Debug.Log($"[DevStorageFurnitureInspectorBuilder] StorageFurnitureInspectorView added under Views. Saved: {PrefabPath}");
            }
    ...
    // ============================================================================
    // Helpers (mirror DevInspectTabBuilder so the new view matches the rest of the tab)
    // ============================================================================
