...
/// <summary>
/// One-shot Editor utility that adds a <see cref="CashierInspectorView"/> GameObject to
/// the DevModePanel prefab as a sibling of the existing inspector views. The view is
/// auto-discovered at runtime by <see cref="DevInspectModule"/> via
/// <c>GetComponentsInChildren&lt;IInspectorView&gt;(true)</c>, so no further wiring is
/// required once this builder runs.
///
/// Layout mirrors <see cref="DevStorageFurnitureInspectorBuilder"/>:
///   • Header label (preferredHeight 28)
///   • ScrollRect + TMP content (flexibleHeight 10, minHeight 200)
///
/// The optional parent-building navigation button is not built here — the runtime view
/// accepts a null <c>_inspectParentBuildingButton</c> gracefully. Wire it manually in
/// the prefab inspector if you want the navigation shortcut.
///
/// Menu:
///   Tools/DevMode/Build Cashier Inspector                 — additive, aborts if it already exists.
///   Tools/DevMode/Rebuild Cashier Inspector (Destructive) — deletes existing then rebuilds.
/// </summary>
public static class DevCashierInspectorBuilder
{
    private const string PrefabPath = "Assets/Resources/UI/DevModePanel.prefab";
    private const string ViewName = "CashierInspectorView";

    [MenuItem("Tools/DevMode/Build Cashier Inspector")]
    public static void BuildCashierInspector()
    {
        RunBuild(destructive: false);
    }

    [MenuItem("Tools/DevMode/Rebuild Cashier Inspector (Destructive)")]
    public static void RebuildCashierInspector()
    {
        bool confirmed = EditorUtility.DisplayDialog(
            "Rebuild Cashier Inspector",
            "This will DELETE any existing 'CashierInspectorView' under DevModePanel/ContentRoot/InspectContent/Views, then rebuild it from scratch.\n\nAre you sure?",
            "Yes, rebuild",
            "Cancel");
        if (!confirmed) return;
    ...
    private static void RunBuild(bool destructive)
    {
        ...
        if (contents == null)
        {
            Debug.LogError($"[DevCashierInspectorBuilder] Could not load prefab at {PrefabPath}");
            return;
        }

        try
        {
            // Path: DevModePanel/ContentRoot/InspectContent/Views
            var contentRootT = contents.transform.Find("ContentRoot");
            if (contentRootT == null)
            {
                Debug.LogError("[DevCashierInspectorBuilder] ContentRoot not found. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            var inspectContentT = contentRootT.Find("InspectContent");
            if (inspectContentT == null)
            {
                Debug.LogError("[DevCashierInspectorBuilder] InspectContent not found under ContentRoot. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            var viewsT = inspectContentT.Find("Views");
            if (viewsT == null)
            {
                Debug.LogError("[DevCashierInspectorBuilder] Views not found under InspectContent. Run 'Tools/DevMode/Build Inspect Tab' first.");
                return;
            }
            ...
            if (existing != null)
            {
                Debug.LogWarning($"[DevCashierInspectorBuilder] '{ViewName}' already exists. Use the destructive variant to overwrite.");
                return;
            }
            ...
            if (referenceFont == null)
            {
                ...
                Debug.LogWarning("[DevCashierInspectorBuilder] No TMP font found in prefab; falling back to TMP_Settings.defaultFontAsset.");
            }
            ...
            // feh=false so Header keeps its preferred 28 px and Content's flexH=10 absorbs all leftover space.
            viewLayout.childForceExpandHeight = false;

            var viewComponent = viewRoot.AddComponent<CashierInspectorView>();
            ...
            // ScrollRect with TMP content (matches DevInspectTabBuilder.CreateScrollRect)
            GameObject scrollGO = CreateScrollRect(viewRoot.transform, "Content", referenceFont, out TMP_Text contentTMP);
            ...
            {
                Debug.LogError("[DevCashierInspectorBuilder] CashierInspectorView is missing expected serialized fields (_headerLabel / _content).");
            }
            else
            {
            ...
            if (!success)
            {
                Debug.LogError("[DevCashierInspectorBuilder] PrefabUtility.SaveAsPrefabAsset returned success=false.");
            }
            else
            {
                Debug.Log($"[DevCashierInspectorBuilder] CashierInspectorView added under Views. Saved: {PrefabPath}");
            }
    ...
    // ============================================================================
    // Helpers (mirror DevInspectTabBuilder / DevStorageFurnitureInspectorBuilder)
    // ============================================================================
